...
\begin{lemma}[Substitution Lemma - Lemma C.10 in M\&Y]\rm
	\begin{enumerate}[1.]
		...
		\item	Suppose $\Gamma \cat x:\chtype{S}; \Lambda; \Delta \proves P \hastype \Proc$ and
			$a \not\in \dom{\Gamma, \Lambda, \Delta}$. 
			Then $\Gamma \cat a:\chtype{S}; \Lambda; \Delta   \vdash P\subst{a}{x} \hastype \Proc$.

		\item	Suppose $\Gamma; \Lambda_1 \cat X:\lhot{S}; \Delta_1  \proves P \hastype \Proc$ 
			and $\Gamma; \Lambda_2; \Delta_2  \proves V \hastype \lhot{S}$ with 
		...
		\item	Suppose $\Gamma \cat X:\shot{S}; \Lambda; \Delta  \proves P \hastype \Proc$ and
