...
Encoding of polyadic into monadic communication (cf.~Defintion \ref{d:enc:poltomon}). 
The mappings 
$\map{\cdot}^\mathsf{p}$,
$\mapt{\cdot}^\mathsf{p}$, 
and 
...
Details in Appendix~\ref{app:polmon}.
	\qed
\end{proof}
...
Encoding of first-order communication into higher-order communication (cf.~Defintion~\ref{d:enc:fotoho}).
The mappings 
$\map{\cdot}^1$,
$\mapt{\cdot}^1$, 
and 
...
\begin{proof}
By induction on the inference $\Gamma; \emptyset; \Delta \proves P \hastype \Proc$. Details in Appendix~\ref{app:enc_sesspnr_to_ho} (Page~\pageref{app:enc_sesspnr_to_ho}).
	\qed
\end{proof}
...
\begin{enumerate}[1.]
%%  Output 
\item Case $P = \bout{k}{k'} P'$,  $\map{P}^{1} = \bbout{k}{ \abs{z}{\,\binp{z}{X} \appl{X}{k'}} } \pmap{P'}{1}$.
Then we have the following typed transition for $\map{P}^{1}$:
$$
\bbout{k}{ \abs{z}{\,\binp{z}{X} \appl{X}{k'}} } \pmap{P'}{1} \hby{\bactout{k}{\abs{z}{\,\binp{z}{X} \appl{X}{k'}}}} \pmap{P'}{1} 
$$
and is easy to see that (i) $\bactout{k}{\abs{z}{\,\binp{z}{X} \appl{X}{k'}}} = \mapa{\bactout{k}{k'}}^{1}$
and (ii)
$$\bout{k}{k'} P' \hby{\bactout{k}{k'}} P'$$
\jp{TBC.}

\item Case $P = \binp{k}{x} Q$,  $\map{P}^{1} =  \binp{k}{X} \newsp{s}{\appl{X}{s} \Par \bbout{\dual{s}}{\abs{x}{\pmap{Q}{1}}} \inact}$ \\
\jp{TBC.}
\end{enumerate}
\qed
\end{proof}
...
%		\auxmapp{P \Par Q}{\mathsf{v}}{\sigma} &\bnfis& \auxmapp{P}{\mathsf{v}}{\sigma} \Par \auxmapp{Q}{\mathsf{v}}{\sigma} 
	\end{array}
\]
\caption{\label{f:auxmap} Auxiliary Map (cf. Def.~\ref{d:auxmap})
...
\begin{proof}
By induction on the inference $\Gamma; \emptyset; \Delta \proves P \hastype \Proc$. 
Details in Appendix~\ref{app:enc_HO_to_sessp} (Page~\pageref{app:enc_HO_to_sessp}).
	\qed
\end{proof}
