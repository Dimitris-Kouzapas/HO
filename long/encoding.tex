...
\begin{proof}
By transition induction, considering Remark~\ref{r:multilabels} for weak transitions. All cases are easy;
...
\noi Part (a). We consider two non-trivial cases, using biadic communication:
\begin{enumerate}[1.]
...
\noi Part (b). We consider two non-trivial cases, using biadic communication:
\begin{enumerate}[1.]
...
$\map{P}^{\mathsf{p}} = 
		...
		and $\ell_2 = \bactinp{k}{k_1,   k_2}$. Then we have, for some $S$, $S_1$, $S_2$, and $\Delta$, the following transitions:
\begin{eqnarray*}
& & \mapt{\Gamma}^{\mathsf{p}}; 
...
\begin{definition}[Finite First-Order into Higher-Order]%\rm
...
%	Define $\encod{\cdot}{\cdot}{1}: \sessp^{-\mu} \to \HO$  as follows:
	...
	\end{array}
	\]
and mappings $\map{\cdot}^{1}$, $\mapt{\cdot}^{1}$, $\mapa{\cdot}^{1}$
