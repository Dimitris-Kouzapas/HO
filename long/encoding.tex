...
In the context of this paper, we consider as typed calculi some
...
\begin{definition}[Typed Encoding]\rm
	...
	$\mapt{\cdot}: T_1 \to T_2$, and $\mapa{\cdot}: \ell \to \ell$, 
	we write 
...
%	for the encoding from $\calc{L_1}{T_1}{\red_1}{\wb_1}{\proves_1}$ to $\calc{L_2}{T_2}{\red_2}{\wb_2}{\proves_2}$.
	$\lrangle{\map{\cdot}, \mapt{\cdot}, \mapa{\cdot}} : \tyl{L}_1 \to \tyl{L}_2$
...
a mapping on types $\mapt{\cdot}$, and a mapping on transition labels $\mapa{\cdot}$.
...
\begin{definition}[Semantic Preserving Encoding]\rm
	\label{def:ep}
	We say that $\lrangle{\map{\cdot}, \mapt{\cdot}, \mapa{\cdot}}: \tyl{L}_1 \to \tyl{L}_2$ is a \emph{semantic preserving encoding}
...
%				$\mapt{\Gamma}; \mapt{\Delta_1} \wb_2 \mapt{\Delta_2} \proves_2 \map{P'} \wb_2 Q$.
			...
			\item Soundness:\\   
			If  $\stytra{\mapt{\Gamma}}{\ell_2}{\mapt{\Delta_1}}{\map{P}}{\mapt{\Delta'_1}}{Q}$
...
thanks to the linearity (and non-interference properties) of session endpoints~\cite{VascoFun}.
...
Thanks to this encoding, %Because of the polyadic to monadic encoding %, denoted  $\auxmap{\cdot}{\mathsf{p}}$,
