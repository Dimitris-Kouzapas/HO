...
Our developments not only recast these classic results into a session typed setting, but 
also determine new encodability results.
...
This is a new result for session typed calculi, whose relevance is stressed by the fact that 
we consider quite demanding criteria for encoding correctness, in particular full abstraction up to characteristic bisimilarity~\cite{characteristic_bis}.
As such, this new result also relevant in a broader setting, for known encodings 
of name-passing into calculi with process- and abstraction-passing either require limitations
in source/target languages and/or fail to satisfy strong encodability criteria (see below). 
...
abstract encodings.  To the best of our knowledge, these are the first
...
is expressive enough to encode both first- and higher-order $\pi$-calculus.
Building upon~\cite{ThomsenB:plachoasgcfhop},
the work~\cite{XuActa2012} studies 
the (non)en\-co\-da\-bi\-lity of the $\pi$-calculus into 
...
In a recent related development, the paper~\cite{DBLP:journals/corr/XuYL15} defines an encoding of the $\pi$-calculus 
without  using of relabeling; names are encoded using polyadic name abstractions. 
The encoding in~\cite{DBLP:journals/corr/XuYL15} does not enjoy full abstraction; 
properties of operational correspondence (completeness and a weak form of soundness) and divergence-reflection are established.


A minimal calculus of higher-order concurrency is studied in~\cite{DBLP:journals/iandc/LanesePSS11}: it lacks restriction,  name passing, output prefix (asynchronous communication), and constructs for infinite behaviour. 
