...
\item[Recursion]
The encoding of a recursive process $\recp{X}{P}$ must preserve the linearity of session endpoints. We
...
``duplicator'' process~\cite{ThomsenB:plachoasgcfhop}. We encode $X$ 
...
\jp{\noi Directly from \figref{f:enc:hopi_to_ho} we may state:
...
%:
We closely follow Sangiorgi's encoding~\cite{San92,SaWabook}, but casted in the setting of session-typed communications. 
%Intuitively, such an encoding  represents 
The exchange of a process is represented in~\cite{San92,SaWabook} with the exchange of a fresh \emph{trigger name}. 
