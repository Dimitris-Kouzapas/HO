...
%\item 
As expected from 
a classical literature in untyped setting \cite{SangiorgiD:expmpa}, 
...
\HOp 
without
name-passing and recursions 
can serve as a new core calculus    
for higher-order session concurrency.  
We call this core calculus as \HO. 
We show \HO can encode \HOp more precisely and efficiently 
than \sessp. In addition, in the session types setting, 
\HO can provide more efficient bisimulation technique 
than \HOp and \sessp. 
%constitute 
%the main sources 
...
\dk{Our full abstraction results are stated
up to two
...
and 
\emph{higher-order bisimilarity} ($\hwb$), which is introduced in this
work.
This $\hwb$ offers more tractable reasoning than $\fwb$. }
