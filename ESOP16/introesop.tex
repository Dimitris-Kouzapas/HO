...
also study compilations (or \emph{encodings}):
...
In this paper we study the type-preservining, 
relative expressivity of \HOp in relation, proposing 
two new criterias, {\em precise} encodability and 
{\em minimum} encodability. 
%to two 
%sub-calculi
%that distill first- and higher-order session-based concurrency. 
%\begin{enumerate}[-]
%\item 
As the expected result from 
a classical literature \cite{SangiorgiD:expmpa} in untyped setting, 
...
%\item 
In this paper, 
our \emph{main discovery} is that 
a new core calculus, 
\HO, which has no name passing and recursion, 
can serve as a core calculus    
...
up to two
typed behavioural equalities that characterize barbed congruence:
\emph{characteristic bisimilarity} ($\fwb$, developed in~\cite{characteristic_bis})
and 
\emph{higher-order bisimilarity} ($\hwb$), which is introduced in this work.
...
%\item 
\secref{sec:calculus} presents \HOp, its 
...
In \secref{ss:negative} we prove the impossibility of encoding communication along shared names
...
%\item 
\secref{sec:relwork} reviews with related works;
