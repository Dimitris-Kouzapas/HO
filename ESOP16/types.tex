...
\noi %Here we define 
This section defines a session type system for \HOp and states its main properties.
Our system distills the key features of~\cite{tlca07,MostrousY15} and so it is simpler.
The main result is \emph{type soundness} (\thmref{t:sr}).
...
%\jpc{functional}
types, respectively.
Session types, denoted by $S$, follow the standard binary session type syntax~\cite{}, with
...
\jparagraph{Typing Rules} The typing system is defined in \figref{fig:typerulesmy}.
