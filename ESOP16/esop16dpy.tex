...
There is a vast literature on expressiveness for process calculi.
...
\newj{Recently the work \cite{OY2016} studies the two encodings,  
from PCF with effect system into session-typed $\pi$-calculus 
and its reverse. The reversed encoding is used to implement 
the session channel passing into Concurrent Haskell. 
As a future work, we plan to use the core calculi 
studied in this paper to implement the higher-order communication 
