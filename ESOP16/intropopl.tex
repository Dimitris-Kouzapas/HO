...
%\newpage
This paper contributes with new expressiveness results in which 
...
There are two key challenges involved in this encoding:
\begin{enumerate}[a.]
\item Although encodings of name-passing into process-passing exist~\cite[Ch.13]{SaWabook}, 
they come with restrictions on name usages;  
such encodings work for %name-passing 
calculi 
%with \emph{capability types} 
in which only the output capability of names is communicated---a received name cannot be used in subsequent inputs.
This is far too limiting in a session typed setting, where types describe \emph{communication structures}, rather than individual name capabilities. % at a given time.
...
Unfortunately, this encoding strategy does not carry over to session typed calculi,
because linear abstractions cannot be copied. That is, the discipline of  session types severely limits 
