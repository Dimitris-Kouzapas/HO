...
names/channels are endowed with session types (such as $T_1$)
representing their intended interactive behavior.
...
Besides offering a natural bridge between concurrent and functional computation, higher-order calculi with session 
primitives enable the specification of practical scenarios involving code mobility.
This way, e.g., the session type 
$T_2 = \btbra{lin:\btinp{\lhot{\mathsf{int}}}\tinact ~ , ~ sha:\btinp{\shot{\mathsf{int}}}\tinact}_{}$
abstracts a server protocol that allows 
  clients to select between two 
kinds of functions from integers to processes (mobile code):
the function
can be either \emph{linear} (to be used exactly once)
or \emph{shared} (to be used without restrictions). 
Therefore,  to interact safely with a server typed with $T_2$, clients must first make a selection (using labels $lin$ and $sha$)
and then they should receive the function itself  and close the protocol afterwards.
...
In this paper we investigate the expressiveness of \HOp in relation to its
...
\end{enumerate}
\HO and \sessp distil the essence of higher- and first-order concurrency with sessions:
while \sessp is, 
