...
Results of relative expressiveness are indispensable ingredients in the principled transference of reasoning techniques, notably behavioral equivalences, across different session languages.
...
%This is a challenging task, as we elaborate next.
 Below, we elaborate on the challenges involved.
...
These patterns are defined as (possibly recursive) sequences of communication actions (send/receive a value, offer/select a behavior).
For instance, session type 
...
higher-order calculi with sessions enable the specification of practical scenarios involving \emph{code mobility}.
...
these are denoted $\lhot{C}$ and $\shot{C}$, respectively, where $C$ is 
a first-order type $C$ (say, a session name).
...
non deterministic, session communication is deterministic and takes place on linear names.
...
%\newpage
This paper contributes with new expressiveness results which allows us to assess the expressivity 
...
an encoding of \HOp into \HO, where both name-passing and recursion is absent (\secref{subsec:HOpi_to_HO}).  
This encoding involves two key challenges:
\begin{enumerate}[a.]
\item Known encodings of name-passing into process-passing~\cite[Ch.13]{SaWabook} are limited in that 
%they come with restrictions on name usages;  
they work for %name-passing 
calculi 
%with \emph{capability types} 
in which only the output capability of names is sent. Hence, a received name cannot be used in subsequent inputs.
This is far too limiting in \HOp, where 
session names for arbitrary protocols must be passed around (\emph{delegation})
...
there is a well-known encoding 
of infinite behavior 
%(recursion and replication)
that exploits process duplication. 
...
Above, $a,b$ are names; $s$ and $\dual{s}$ are 
