...
%defined as
$\abs{x_1\cdots x_k}{\auxmapp{P}{{}}{\emptyset} }$, where $\vmap{m_j} = x_j$, for all $j \in \{1, \ldots, k\}$. 
...
\newj{Recently, the work \cite{OY2016} studies two encodings:
from PCF with an effect system into a session-typed $\pi$-calculus, 
and its reverse. The reverse encoding is used to implement session channel passing into Concurrent Haskell. 
