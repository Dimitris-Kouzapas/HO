...
\newc{Below, shared value means that there are no free linear names, thus $\Lambda, \Delta$ are empty (cf. Rule $\trule{Prom}$ in \figref{fig:typerulesmy}).}
...
\begin{lemma}[Substitution Lemma - Lemma C.10 in~\cite{MostrousY15}]\rm
	...
	\begin{enumerate}[1.]
		...
		\item	Suppose $\Gamma \cat x:\shot{U}; \Lambda; \Delta  \proves P \hastype \Proc$ and
			shared value $V$ such that $\Gamma; \emptyset ; \emptyset  \proves V \hastype \shot{U}$
...
the 
definition of structural congruence in~\cite{MostrousY15} is richer than ours. 
...
Since we are working with synchronous communication this ordering can be omitted. 
The second part of the following statement corresponds to \thmref{t:sr} (Page~\pageref{t:sr}):
...
%	   }
	   ...
	   $$
	    ...
	    $\Gamma$ to stand for $ \Gamma' \setminus x$).
	    \[	    \hspace{-12mm}
	    \tree{
	     \eqref{eq:sound3}
%	    \tree{
...
%	   }
 ...
 & 	    \label{eq:sound4}
	    ...
	    $\Gamma' \backslash x;\, \emptyset ;\, \Delta_3    \proves   Q_2  \hastype \Proc$
	    and
	    $\Gamma;\, \emptyset ;\, \emptyset  \proves V  \hastype L$
	    we obtain 
	   $$
