...
\dk{
	...
	recursive type.
	\begin{example}
		Let recursive type $S = \trec{t}{ \btout{S'} \vart{t}}$ with
		$\mapchar{S}{s} = \mapchar{ \btout{S'} S }{s} = \bout{s}{s'} \bout{t}{s} \inact$. If we type the
		characteristic process for $S$ we get:
		\[
...
\begin{proof}[Sketch]
\dk{
	The main difference between the definition of the two bisimilarity relations
