...
Notice that due to weakening of shared environments we have 
$
...
%
\noi
Process in \eqref{eqb:0} is called higher-order trigger process,
while process in \eqref{eqb:4} is called characteristic trigger process.
Notice that while 
in \eqref{eqb:0} there is a higher-order input on $t$, 
in \eqref{eqb:4} variable $x$ does not play any role.

We use the higher-order trigger process to define the higher-order bisimilarity:
...
We use the characteristic trigger process to define the higher-order bisimilarity: 
...
\subsection{Determinstic Transitions and Up-to Techniques}
...
\begin{proof}[Sketch]
	...
	\betatran and session-transition. The proof for Part 2 is direct from Part 2.
	See \appref{app:sub_tau_inert} for the full details of the proof.
	\qed
\end{proof}
...
\begin{proof}[Sketch]
	...
	\lemref{app:lem:proc_subst} in \appref{app:sub_coinc}.
	\qed
\end{proof}
...
\begin{proof}[Sketch]
	The proof relies on \lemref{lem:proc_subst} to prove the that:
	\begin{enumerate}
		\item	whenever two processes are higher-order bisimilar
				...
				requirement for context bisimilarity.

		\item	the input requirement is then further used
				to prove that the output clause for $\hwb$
...
%
	The full proof is found in
	\lemref{app:lem:wb_is_wbc} in \appref{app:sub_coinc}.
	\qed
\end{proof}
...
\begin{proof}[Sketch]
	...
	To show parallel congruence we construct
...
%
	...
	\end{eqnarray*}
%
	It remains to show that $\mathcal{S}$ is a context bisimulation
	by doing a case analysis on the transitions of the pairs in $\mathcal{S}$.
	The full proof is found in
	\lemref{app:lem:wbc_is_cong} in \appref{app:sub_coinc}.
	\qed
\end{proof}
...
\begin{proof}[Sketch]	
	...
	a set of test processes
	$\Gamma; \emptyset; \Delta_2 \proves T\lrangle{\ell, \suc} \hastype \Proc$
	with the property:
%
	\[
		P \Par T \red P' \Par \bout{\suc}{\dual{m}} \inact
		\quad \mbox{if} \quad
		\Gamma; \Delta \proves P \hby{\ell} \Gamma; \Delta' \proves P'
	\]
%
	(see \defref{app:def:definibility} for the proper definition).
	The test process can be used to check the
	labelled transition interactions of
	reduction-closed barb congruence processes,
	$\Gamma; \Delta \proves P\ \cong\ \Delta' \proves Q$,
	and associate them with 
	the defining properties of context bisimulation
	to show that
	$\Gamma; \Delta \proves P\ \wbc\ \Delta' \proves Q$,.
	The full details can be found in \lemref{app:lem:cong_is_wb} in \appref{app:sub_coinc}.
	\qed
\end{proof}
