...
\begin{proof}[Sketch]
	...
	case (see \lemref{app:lem:subst_equiv} in \appref{app:beh});
	It proves the result by constructing a closure on the above
	properties and proving that it is a higher-order bisimulation.
%	by using an up-to determinate technique; 
	The full proof is found in
	\lemref{app:lem:proc_subst} in \appref{app:beh}.
	\qed
\end{proof}
