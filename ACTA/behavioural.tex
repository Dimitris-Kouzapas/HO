...
%	\]
	...
	\end{eqnarray*} 
	Characteristic processes have the following property: 
	\[
		\mapchar{U}{n} \by{\ell} \bout{t}{n} \inact 
	\]
	\JP{But this is only true when $U$ is a session type different from $\tinact$, right? Is this an issue when $U$ is an arrow type?}
	By the last property we can always observe (note that below $\ell_1$ may be an action $\tau$, thus denoting
...
\JP{JP: The observable actions do not concern the refined LTS. Also, it seems that the actions 
