...
\dk{A typed transition relation requires type judgements with an empty $\Lambda$
environment. This is for simplicity because we can always apply typing 
rule $\trule{EProm}$ and get an empty $\Lambda$. Furthermore, recall that
we are working with closed terms.}
...
\dk{Observing the outputs is enough because input actions can be observed indirectly by output actions 
on fresh names. For instance, the process $P = \binp{n}{x} P’$ has an input barb; by composing $P$
with  $\bout{n}{m} \bout{\suc!}{} \inact$ (with fresh $\suc$) then one obtains a (weak) observation
uniquely associated to  the input in $P$. }
...
\[
	\begin{array}{rclcrcl}
		\mapchar{\btinp{U} S}{u}
		&\defeq&
		\binp{u}{x} (\binp{t_1}{y} \appl{y}{u}  \Par \binp{t_2}{y}\newsp{s}{\appl{y}{s} \Par \bout{\dual{s}}{x} \inact})
		&\qquad &
		\omapchar{S}  & \defeq &  s ~~ (s \textrm{ fresh})
		\\
		\mapchar{\btout{U} S}{u}
		&\defeq&
		\bout{u}{\omapchar{U}} \binp{t}{y} \appl{y}{u} 
		&&
		\omapchar{\chtype{S}} \defeq \omapchar{\chtype{L}} &\defeq&a ~~ (a \textrm{ fresh})
		\\
		\mapchar{\btsel{l : S}}{u}
		& \defeq &
		\bsel{u}{l} \binp{t}{y} \appl{y}{u}
		&&
		\omapchar{\shot{U}} \defeq \omapchar{\lhot{U}} & \defeq &  \abs{x}{\mapchar{U}{x}}
		\\
		\mapchar{\btbra{l_i: S_i}_{i \in I}}{u}
		& \defeq &
		\bbra{u}{l_i: \binp{t}{y} \appl{y}{u}}_{i \in I}
		&&
...
%		\recp{X_{\vart{t}}}{\mapchar{S}{u}}
		...
		\mapchar{\chtype{S}}{u} 
		&\defeq&
		\bout{u}{\omapchar{S}} \binp{t}{y} \appl{y}{u} %\inact
		& & 	
		...
		\mapchar{\chtype{L}}{u}
		&\defeq&
		 \bout{u}{\omapchar{L}} \binp{t}{y} \appl{y}{u}%\inact
		&&
...
\begin{definition}[Refined Typed Labelled Transition System]
	...
	is 
	defined by the all
	rules in \figref{fig:envLTS} excepting Rules $\eltsrule{SRv}$ and $\eltsrule{ShRv}$, 
	which are replaced by the following one:
	\begin{mathpar}
		\inferrule[\eltsrule{RRcv}]{
			(\Gamma_1; \Lambda_1; \Delta_1) \by{\bactinp{n}{V}} (\Gamma_2; \Lambda_2; \Delta_2)
			\and
...
\begin{proof}[Sketch]
\dk{	The only difference in two bisimilarity relations definition
	is the trigger process. Thus, the interesting case is the
	case where we observe an output from a process. The
	...
	given that 
	\[
		\newsp{\tilde{m_1}}{P_1 \Par \ftrigger{t}{V}} \fwb \newsp{\tilde{m_2}}{P_2 \Par \ftrigger{t}{V_2}{U}}
	\]
	when showing that $\hwb \subseteq \fwb$.
}
...
\begin{proof}[Sketch]
	...
	need to use \lemref{lem:trigger_subst} where
	we show that if
	\[
...
%	higher-order bisimulation, up-to \betatran (\lemref{lem:up_to_deterministic_transition}).
	...
	\lemref{lem:process_subst} (\mypageref{lem:process_subst}).
	\qed
\end{proof}
