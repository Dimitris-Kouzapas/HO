...
\begin{proof}[Sketch]
	\dkmargin{please check this sketch proof}
    The full proof, found in \appref{app:sub_coinc}, 
	\lemref{lem:process_subst} (\mypageref{lem:process_subst}), is obtained 
by constructing a typed relation on the
	...
	Following the higher-order bisimulation requirements we use an auxiliary result 
	(\lemref{lem:trigger_subst}) which shows that if
	\[
	...
	\]
	In turn, the proof for \lemref{lem:trigger_subst} follows the definition of the characteristic process.
	We need to construct a typed relation:
	\begin{eqnarray*}
		\Re &=& \set{
					\horel{\Gamma}{\Delta_1'}{\newsp{\widetilde{m_1}}{P \Par \appl{(\abs{x}{R})}{n_1} }}
					{\ ,\ }
	...
	\end{eqnarray*}
	We sketch the proof that $\Re$ is a higher-order bisimulation.
	Assume that
	\begin{eqnarray*}
