...
barbed congruence (denoted $\cong$, cf. \defref{def:rc}), the form of contextual equivalence used in concurrency.
This characterisation result is given in 
...
As it will be clear below (cf. \defref{d:typedrel}), we will be working with closed process terms, i.e., processes without free  variables.
}
...
\begin{definition}[Session Environment Confluence]\label{d:conf}
	...
	if there exists a $\Delta$ such that:
...
\begin{example}[Session Environment Confluence]
	Consider the (balanced) session environments:
	\begin{eqnarray*}
	\Delta_1 & = & \set{s_1: T_1 \cat s_2: \btinp{U_2} \tinact \cat \dual{s_2}: \btout{U_2} \tinact} \\
	\Delta_2 & = & \set{s_1: T_1 \cat s_2: \btout{U_1} \btinp{U_2} \tinact \cat \dual{s_2}: \btinp{U_1} \btout{U_2} \tinact}
	\end{eqnarray*}
	Following \defref{d:wtenvred}, we have that 
 $\Delta_1 \red \set{s_1: T_1 \cat s_2: \tinact \cat \dual{s_2}: \tinact}$
	and $\Delta_2 \red \red \set{s_1: T_1 \cat s_2: \tinact \cat \dual{s_2}: \tinact}$.
	Therefore,  $\Delta_1$ and $\Delta_2$ are confluent.
	\qed
\end{example}

Typed relations relate only closed processes whose
session environments are balanced  and confluent:

\begin{definition}[Typed Relation]\label{d:typedrel}
...
with  $\bout{n}{m} \bout{\suc}{} \inact$ (with a fresh name $\suc$) then one obtains a (weak) observation
...
\noi The first behavioural relation that we define is reduction-closed, barbed congruence~\cite{HondaKYoshida95}. 
...
processes with first- and higher-order communication.
%$\hwb$ and  $\fwb$.
...
\newc{
	...
	recursive type.
	\begin{example}[Characteristic process for a recursive session type]
	Consider the type 
...
\[
	\begin{array}{rcll}
		\Gamma; \es; \Delta &\proves& P_1 \by{\bactinp{s}{\abs{x} \newsp{s'}{\bout{a}{s'} \inact}}} \by{~\tau~} \by{~\tau~}\\
		\Delta &\proves& \newsp{s'}{\bout{a}{s'} \inact} \Par \newsp{s'}{\bout{a}{s'} \inact}
		\\[4mm]
		\Gamma; \es; \Delta &\proves& P_2 \by{\bactinp{s}{\abs{x} \newsp{s'}{\bout{a}{s'} \inact}}} \by{~\tau~}\\
...
%	$\abs{z}{\binp{z}{x} (\appl{x}{m})}$,
	...
	then, under appropriate $\Gamma$ and $\Delta$, they would become:
%
%	\begin{eqnarray*}
...
Observe how we have used \notref{not:outtype} to explicitly refer to the type of the emitted value in output actions.
...
%
\begin{proof}[Sketch]
	The proof of Part 1 requires to show that relation (we omit type information)
	\[
		\Re = \set{(P, P') \setbar \Gamma; \Delta \proves P \hby{\dtau} \Delta' \proves P'}
	\]
	is a higher-order bisimulation.
%	The proof for Part 1 requires a case analysis on the structure of \betatran and session-transition.
	The proof for Part 2 is direct from Part 1.
	See \appref{app:sub_tau_inert} (\mypageref{app:sub_tau_inert}) for the details.
	\qed
\end{proof}
...
%	the definition of the two bisimilarity relations
	is the trigger process (higher-order triggers $\htrigger{t}{V}$ in $\hwb$ and characteristic triggers $\ftrigger{t}{V}{U}$ in $\fwb$). Thus, the most interesting case in the proof is when we observe an output from a process. 
	...
	\]
	The proof for the above equalities is coinductive, exploiting the freshness of the trigger name in each case; see  \lemref{lem:trigger_equiv} in the Appendix.
	While the proof of the first equality (i.e., higher-order triggers imply characteristic triggers) follows expected lines, the proof of the second 
	equality 
	(i.e., characteristic triggers imply higher-order triggers) is a bit more involved. 
	Indeed, while higher-order trigger processes can input trigger values, 
	characteristic triggers cannot. 
	However, we prove that this does not represent a difference in behaviour; see case 2(c) in 
	\lemref{lem:trigger_equiv}.
	To this end, we exploit an alternative trigger process, denoted $\ntrigger{t}{V}$, simpler than the 
	higher-order trigger $\htrigger{t}{V}$ in \eqref{eqb:0}:
\begin{equation*}
	\ntrigger{t}{V} = \newtrigger{t}{V}
%	\label{eq:ntrig}
\end{equation*}
In the proofs for these coincidence results, we exploit some auxiliary results for trigger processes, including 
  a two-way connection between $\htrigger{t}{V}$ and $\ntrigger{t}{V}$
   (cf. \lemref{lem:alt_tr} (3) in the Appendix).
	We thus infer that 
...
%	\end{eqnarray*}
%%
	In turn, using the above results we can show that typed relations induced by 
...
%	\]
	...
	Let us consider the particular case in which $I$ is a singleton; we then construct a typed relation $\Re$:
	\begin{eqnarray*}
		\Re &=& \set{
					\horel{\Gamma}{\Delta_1'}{\newsp{\widetilde{m_1}}{P \Par \appl{(\abs{x}{R})}{n_1} }}
					{\ ,\ }
	...
	\end{eqnarray*}
	The typed relation $\Re$ can be shown to be a higher-order bisimulation
...
%	Is this an issue when $U$ is an arrow type? \dk{No it is not an issue - you can check that either type has the same transition}}
	By the last property we can always observe, for some $\Delta''_3$ (note that below $\ell_1$ may be an action $\tau$, thus denoting
...
We now show that higher-order bisimilarity is sound with respect to context bisimilarity.
...
Now we revisit our running example to prove that $\Client_1$ and $\Client_2$ 
