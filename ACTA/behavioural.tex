...
it reduces the session environment or keeps it 
unchanged (\defref{d:wtenvred}).
...
Notice that for open process terms (i.e., with free process variables), 
we can always apply Rule~$\trule{EProm}$ (cf. \figref{fig:typerulesmy}) and obtain an empty $\Lambda$. 
As it will be clear below (cf. \defref{d:typedrel}), we will be working with closed process terms, i.e., processes with no free process variables.
}
...
with  $\bout{n}{m} \bout{\suc}{} \inact$ (with a fresh $\suc$) then one obtains a (weak) observation
...
%	\]
	...
	Let us consider the particular case in which $I$ is a singleton; we construct a typed relation $\Re$:
	\begin{eqnarray*}
		\Re &=& \set{
					\horel{\Gamma}{\Delta_1'}{\newsp{\widetilde{m_1}}{P \Par \appl{(\abs{x}{R})}{n_1} }}
					{\ ,\ }
