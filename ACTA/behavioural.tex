...
%	\]
	...
	appears, $\bout{t}{n} \inact$ where $n$ can be either a shared or a session name. 
	...
	\end{eqnarray*}
	Characteristic processes have the property that
	\[
		\mapchar{U}{n} \by{\ell} \bout{t}{n} \inact 
	\]
	By the last property we can always observe (note that $\ell_1$ might be action $\tau$ denoting
...
%		= \binp{s}{x} (\inact \Par \appl{x}{k})
%		\scong \binp{s}{x} (\appl{x}{k}) 
\quad\text{\newc{JP: Why is this relevant here??}}
	$$
...
In the following analysis, we use the following shorthand notation:
%	\begin{eqnarray*}
$$
...
%
$$
A detailed account of the observable behavior of 
$\Client_1$ is given in \figref{f:cli1}; similarly, \figref{f:cli2}
illustrates the actions possible from $\Client_2$, which are the same as for $\Client_1$.
\newc{JP: The observable actions do not concern the refined LTS. Also, it seems that the actions 
use characteristic triggers, not the HO triggers of $\hwb$. Clearly both relations coincide, but we should mention it.}
...
%		& \Par \binp{t_2}{x} \newsp{s}{\mapchar{\btinp{\lhot{S}}}{s} \Par \bout{\dual{s}}{\abs{x}{P \subst{h_2}{y}}} \inact }}
	...
	\end{array}
\]
\caption{Observable actions from $\Client_1$. \label{f:cli1}}
\end{figure}
...
\\[1mm]
	...
	\end{array}
\]
\caption{Observable actions from $\Client_2$. \label{f:cli2}}
\end{figure}
\qed
\end{proof}
