...
\newc{This notion extends to value types as expected; we write $U_1 \sim U_2$ to denote that $U_1$ and $U_2$ are type-equivalent.}
...
Rule $\trule{\textsc{EProm}}$ allows to freely use a linear
...
\end{eqnarray*}
While the type for $\abs{x}{P_{xy}}$ is $\es; \es; y: S \proves \abs{x}{P} \hastype \lhot{U}$,
