...
synchronisation on shared names, 
  ...
  and recursion.
Thus, \HOp   may specify %reciprocal exchanges (protocols) 
protocols
...
their solution, however, does not consider \emph{linearity} as important in session-based concurrency.

%\smallskip

\paragraph{This Work.}
Building upon~\cite{SangiorgiD:expmpa,San96H,JeffreyR05}, 
our discovery is that  {linearity} of session types plays a vital role 
%to 
...
\secref{sec:relwork}~discusses with related works. 
...
The current presentation includes full technical details---definitions and proofs, collected in the Appendix.
...
given in~\cite{kouzapas_et_al:LIPIcs:2015:5365} (the Hotel Booking scenario) and develop an additional example, given in  
\secref{sec:relwork}: an empirical comparison of our approach with that in~\cite{JeffreyR05}. 
