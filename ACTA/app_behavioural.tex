...
(denoted $\bout{u}{\tilde{V}}{P}$ and  $\binp{u}{\tilde{x}}{P}$)
...
the rest of this section to represent matching actions.

\begin{definition}
Let $\ell$ be an action label (cf. \secref{ss:lts}). We define the action $\mact{l}$ as 
	\[
		\mact{\ell} =
		\left\{
		\begin{array}{lcl}
...
Thus, given $\ell$, its corresponding action $\mact{\ell}$ is either identical to $\ell$, or an output on the same name, with different object and extruded names.
	...
	\begin{enumerate}[a)]
		\item	First direction. Consider the typed relation (we omit the type information):
				...
				\[
					...
					{\Delta_1'}{\newsp{\widetilde{m_1}'}{P' \Par \newsp{s}{\bout{t}{s} \inact}  } }
				\]
				which, from the freshness of $t$, implies that there exists $Q'$
		...
		\item First direction. Let $\Re$ be the typed relation (we omit the type information):
				...
				\begin{enumerate}[i)]
					...
					\item	An input action of the form $\bactinp{t}{n}$ along fresh name $t$.
							In this case, we have that there exists $U$ such that $\omapchar{U} = n$:
							$$
							\begin{array}{rcl}
...
%							\JP{Why $Q$ doesn't move to some $Q'$ in the above weak transition?} 
						...
						\item	An action of the form $\bactinp{t}{\abs{z}{\mapchar{U'}{z}}}$ along fresh name $t$.
								This means that there exist $U, \Delta_1'$ such that $\omapchar{U} = \abs{z}{\mapchar{U'}{z}}$ and
							\[
							\begin{array}{c}
								\horel{\Gamma}{\Delta_1}{\newsp{\widetilde{m_1}}{P \Par \htrigger{t}{V_1}  } }
								{\hby{\bactinp{t}{\abs{z}{\mapchar{U'}{z}}}}} 
								{\Delta_1'}{\\ \qquad \qquad \qquad \qquad \qquad \newsp{\widetilde{m_1}'}{P \Par \newsp{s}{\binp{s}{y} {(\appl{(\abs{z}{\mapchar{U'}{z}})}{y}}) \Par \bout{\dual{s}}{V_1} \inact}}}
								\end{array}
							\]
							Furthermore, we can see that
							\[
