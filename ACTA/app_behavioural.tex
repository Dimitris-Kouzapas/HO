...
%									{\hby{\bactinp{t}{\map{\btinp{U} \tinact}^{x}}}}& & &
		...
		\item	Consider the typed relation (for readability, we omit type information):
				...
				The result is then a consequence of \lemref{lem:alt_tr} (Part 3), where
...
We analyse each part separately:
	\begin{enumerate}[1.]
		\item	The proof for Part 1 is done by contradiction.
				...
				with $\subj{\ell_1} = n_1$ \JP{Why? I think $\ell_1$ can come from $P$ and be different from $n_1$..}, then from the freshness of $t$
				\[
...
$\ell$ is an higher-order output; $\ell$ is a first-order output. \JP{What about higher-order input?}
	\begin{enumerate}[1.]
...
%					\horel{\Gamma}{\Delta_1'}{P_2 \subst{\abs{\widetilde{x}}{\mapchar{U}{\widetilde{x}}}}{x}}{\hwb}{\Delta_2'}{Q_2 \subst{\abs{\widetilde{x}}{\mapchar{U}{\widetilde{x}}}}{x}}\\
 ...
 transition \eqref{lem:wb_is_wbc1} and $\Re$ imply the existence of a process $Q_2$ and name $m_2$ such that 
			...
			\]
			 for some fresh $t$.
				\noi From the  second case of this proof \JP{It is not clear what case you are referring to here} we can conclude that for all $R$ with $\fpv{R} = \set{x}$:
			%
			\[
				\begin{array}{rcl}
					\Gamma; \es; \Delta_1' \proves \newsp{\widetilde{m_1}'}{P_2 \Par \htrigger{t}{m_1}} 
		...
		\item	Case $\ell = \news{\widetilde{m_1}'} \bactout{n}{\abs{\widetilde{x}}{P}}$: This case is similar to the previous case but makes use of the alternative trigger, $\ntrigger{t}{V}$ (cf.~\eqref{eq:ntrig}).
			...
			\[
				...
				{\Delta_2''}{}{\newsp{\widetilde{m_1}'}{Q_2' \Par \newsp{s}{\appl{(\abs{y}{\binp{t'}{x} (\appl{x}{y})) }{s}}  \Par \bout{\dual{s}}{\abs{\widetilde{x}}{Q}} \inact } }}.
			\]
				\noi From the  second case of this proof \JP{It is not clear what case you are referring to here} we can conclude that :
			%
			\[
...
%
	...
	\item	{\bf Barb Preserving:} Suppose $\Gamma; \es; \Delta_1 \proves P \barb{m}$. We analyse three cases, depending on the nature of $m$:
		...
		\item	{\bf Congruence:}
				...
				\end{eqnarray*}
		%
				We show that $\mathcal{C}$ is a congruence with respect to parallel composition. \JP{Perhaps we mean: ``We show that $\mathcal{C}$ is a reduction-closed barbed congruence?}
				We distinguish two cases:
				\begin{enumerate}[i)]
