...
%				\JP{Why do we skip the tau related to beta-reduction?}
				with $C = \inact$ if $\ell$ is not an input action,
...
%>>>>>>> aef27afeb4589c34c9d95590db19c7f7ff07237c
										...
										\[
											\mhorel{\Gamma}{\Delta_1''}{\newsp{\widetilde{m_1}''}{P \subst{n}{x} \Par \mapchar{U}{x} \subst{n_1}{x}}}
											{ \hby{\tau} }
											{\Delta_1'''}{}{\newsp{\widetilde{m_1}''}{P_2 \Par C_1 \Par \subst{n}{x} \Par \bout{t'}{n_1} \inact}}
										\]
										where $C_1 = \inact$ when the action on $\mapchar{U}{x} \subst{n_1}{x}$ is not an input
