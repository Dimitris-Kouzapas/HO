...
\begin{proof}[Sketch]
	\begin{enumerate}
		\item	The proof for part 1 uses the results from Part 2 and 3.
			...
			\begin{enumerate}
				...
				\item	$U = \chtype{L}$.
						...
						The proof is straightforward from the typing rules $\trule{Sess}$ and $\trule{Sh}$.

				\item	$U = \shot{U'}$. By the parts 2 and 3 of this proof we get that
						$\Gamma; \es; \Delta \proves \mapchar{U'}{x} \hastype \Proc$ which implies by
						typing rules $\trule{Abs}$ and $\trule{EProm}$ that
						$\Gamma \backslash x ; \es; \Delta \backslash x \proves \omapchar{\shot{U'}} \hastype \shot{U'}$.

				\item	$U = \lhot{U'}$. Similar using type rule $\trule{Abs}$.
			\end{enumerate}
		\item	The proof of part 2 is done by induction on the the syntax of $S$.
			...
			\begin{enumerate}
				...
				\item	Case $S = \btinp{S_1} S_2$. Then, by \defref{def:char}, we have 
						...
						\]
					\item	Case $S = \trec{t}{S'}$. Then, by \defref{def:char}, $\mapchar{S' \subst{\tinact}{\vart{t}} }{u}$.
		...
		\item	The proof for Part 3 uses the results of Parts 1.
			...
			\begin{enumerate}
				...
				\item	$U = \chtype{S}$. Similar argumentation as the previous case.

				\item
						$U = \lhot{U'}$ then from Part 1 we get that
						$\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'$.
						By applying typing rule $\trule{App}$ and rule $\trule{EProm}$ we get
						\[
							\tree {
								\tree {
									\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'
				...
				\item	$U = \shot{U'}$. Similar argumentation with the previous case
						without applying rule $\trule{EProm}$.
			\end{enumerate}
	\end{enumerate}
	\qed
\end{proof}
