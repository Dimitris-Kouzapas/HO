	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item	If $S = \trec{t}{S'}$ then whenever
					...
					then $\exists S_i, i \in I$, 
					$\Gamma; \es; \Delta \cat {t_i: \btout{S_i}}_{i \in I} \tinact \cat s: \trec{t}{S'} \proves \mapchar{\trec{t}{S'}}{s} \hastype \Proc$
					\dk{$\forall i, S \red S_i$ where $red$ (needs to be defined) consumes a session type prefix, e.g. $\btbra{l_i: S_i}_{i \in I} \red S_k, k \in I$}
...
\begin{proof}
		...
		\item	Second direction. Consider typed relation (we omit the type information for readability)
				...
				\begin{enumerate}
					...
					\item	\[
							...
							up to deterministic transitions $\bactinp{t'}{\abs{z}{\tinact}} \dtau$.
							\begin{eqnarray*}
								&&\Gamma; \es; \Delta_1' \proves \newsp{\widetilde{m_1}}{P \Par \htrigger{t'}{s}  } \\
								\hby{\bactinp{t'}{\abs{z}{\tinact}}} &&
								\Delta_1' \proves \newsp{\widetilde{m_1}}{P \Par \newsp{s'}{\binp{s'}{y} \appl{\inact}{y} \Par \bout{\dual{s'}}{s}}}
								\\
								\hby{\dtau} &&
								\Delta_1' \proves \newsp{\widetilde{m_1}}{P}
							\end{eqnarray*}
	...
	The proof for Part 2 relies on Part 1. We do an analysis on the two
...
%									{\hby{\bactinp{t}{\map{\btinp{U} \tinact}^{x}}}}& & &
		...
		\item	Part 2 
				...
				The result is then a consequence of \lemref{lem:alt_tr} (Part 2).
