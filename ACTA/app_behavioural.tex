...
several lemmas:
\begin{enumerate}[$-$]
\item	\lemref{app:lem:wb_eq_wbf} establishes $\wb\ =\ \wbf$.
...
\item	\lemref{app:lem:cong_is_wb} shows  that $\cong \subseteq \wb$.
\end{enumerate}
...
%	\noi From Lemma~\ref{app:lem:subst_equiv} we get that $\forall R$ with $\fv{R} = x$
%
	\[
...
%		{\Delta_2''}{}{\newsp{\widetilde{m_2}'}{(Q_1' \Par Q_2') \subst{\abs{\widetilde{x}}{R}}{x} \Par \appl{y}{\widetilde{m}} \subst{\abs{\widetilde{x}}{R'}}{y}}}
	\]
	\noi as required.
%	\noi From here we apply Lemma~\ref{app:lem:subst_equiv} for each substituting instance of
...
%
	...
	\]
%
	\noi Furthermore from~(\ref{lem:wbc_is_cong2}) and the definition of $\mathcal{S}$ we conclude that
	\[
...
%
	...
	\]
%
	\noi Furthermore from~(\ref{lem:wbc_is_cong4}) we conclude that $\forall Q$ with $\set{x} = \fpv{Q}$
%
	\[
...
%
	...
	\]
%
	\noi Furthermore from~(\ref{lem:wbc_is_cong6}) we conclude that $\forall Q, x = \fn{Q}$
%
	\[
...
\begin{proof}
	\noi	\lemref{app:lem:wb_eq_wbf} proves $\wb\ =\ \wbf$.
			\lemref{app:lem:cong_is_wb} proves $\cong\ \subseteq\ \wb$.
			\lemref{app:lem:wb_is_wbc} proves $\wb\ \subseteq\ \wbc$.
