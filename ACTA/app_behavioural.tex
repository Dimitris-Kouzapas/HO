...
\begin{definition}[Session Type Prefix Deletion]
	...
	\begin{array}{c}
		...
		\qquad
		\unfold{\btsel{l: S}} = \btsel{l: S}
		\\
		\unfold{\btbra{l_i: S_i}_{i \in I}} = \btbra{l_i: S_i}_{i \in I}
		\qquad 
		\unfold{\tinact} = \tinact
		\qquad
		\unfold{\trec{t}{S}} = \unfold{S \subst{\trec{t}{S}}{t}  }
	\end{array}
	\]
\end{definition}
...
%					$\Gamma \backslash x ; \es; \Delta \backslash x \proves \omapchar{\shot{U'}} \hastype \shot{U'}$.
		...
		\begin{enumerate}
			...
			\item	If $S = \btbra{l_i: S_i}_{i \in I}$ then \\
					$\es; \es; \set{t_i: \btout{S_i}}_{i \in I} \cat s: \btbra{l_i: S_i}_{i \in I} \proves \mapchar{\btbra{l_i: S_i}_{i \in I}}{s} \hastype \Proc $.
		...
		\begin{enumerate}
			...
			\item	If $U = \shot{U'}$ then  $\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'$ then
					$\Gamma \cat x: \shot{U'}; \es;\Delta \proves \mapchar{\shot{U'}}{x} \hastype \Proc$.
	
			\item	If $U = \lhot{U'}$ then   $\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'$ then
