...
\jp{Below I slightly modified the statement and added some structure (enumerate).
...
%
	\[
		\horel{\Gamma}{\Delta_1}{\newsp{\widetilde{m_1}}{P_1 \subst{\abs{\widetilde{x}}{R}}{x} \Par P_2\subst{\abs{\widetilde{x}}{R}}{x} }}{\hby{\ell_1}}{\Delta_1'}{P_1'}
	\]
%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	%          Case
...
%
	...
	\end{eqnarray}
%
	\noi From~\eqref{lem:subst_equiv1} and~\eqref{lem:subst_equiv3} we obtain that $\forall R$ with $\fv{R} = \widetilde{x}$:
%
	\[
		\horel{\Gamma}{\Delta_2}{\newsp{\widetilde{m_2}}{Q_1 \Par Q_2 \subst{\abs{\widetilde{x}}{R}}{x}}}
		{\Hby{\ell_2}}
	...
	\]
%
	\noi The case concludes if we combine~\eqref{lem:subst_equiv2} and~\eqref{lem:subst_equiv4}, to obtain that $\forall R$ with $\fv{R} = \widetilde{x}$
%
	\[
...
%		{\Delta'}{}{\newsp{\widetilde{m_2}'}{(Q_1' \Par Q_2') \subst{\auxtr{t}}{x} \Par \appl{y}{\widetilde{m}} \subst{\abs{\widetilde{x}}{\binp{t'}{y} \appl{y}{\widetilde{x}}}}{y}}}
	\]
%
	\noi From \lemref{app:lem:subst_equiv} we get that $\forall R$ with $\fv{R} = \widetilde{x}$
%
	\begin{eqnarray*}
...
%
	...
	\end{eqnarray}
%
We distinguish four cases: $\ell$ is not an output action; $\ell$ is a higher-order output without name extrusion; $\ell$ is a higher-order output with name extrusion; $\ell$ is a first-order output.
	\begin{enumerate}
		\item
...
\jp{I would write the appropriate closure. Why do we distinguish three sub-cases (why are they important)?
Also, it seems that we consider three cases...

At some point we use $\Red$, I think this is a typo.
}
