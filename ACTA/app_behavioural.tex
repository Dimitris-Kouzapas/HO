...
\begin{proposition}[Characteristic Processes/Values Inhabit Their Types]
	...
	\begin{enumerate}
		...
		\item	If $U = \shot{U'}$ then whenever $\exists \Gamma, \Delta$ such that
				...
				$\Gamma \backslash x ; \es; \Delta \backslash x \proves \omapchar{\shot{U'}} \proves \shot{U'}$.

		\item	If $U = \shot{U'}$ then whenever $\exists \Gamma, \Delta$ such that
				$\Gamma; \es; \Delta \proves \mapchar{U'}{x} \hastype \Proc$ then
				$\Gamma \backslash x ; \es; \Delta \backslash x \proves \omapchar{\shot{U'}} \proves \shot{U'}$.

		\item	If $U = \btout{U'} S$ then whenever
		...
		\item	If $U = \btinp{U'} S$ then whenever
				$\Gamma; \es; \Delta \proves \mapchar{U'}{x}  \hastype \Proc$
				and $\Gamma; \es; \Delta' \cat s: S \proves \mapchar{S}{s} \hastype \Proc$ then
				$\Gamma \backslash x; \es; (\Delta\backslash x) \cat t: \btinp{S} \tinact \cat s: \btout{U'} S \proves \mapchar{\btinp{U'} S}{s} \hastype \Proc$.

		\item	If $U = \btsel{l: S}$ then whenever
				$\Gamma; \es; \Delta' \cat s: S \proves \mapchar{S}{s} \hastype \Proc$ then
				$\es; \es; t: \btout{S} \tinact \cat s: \btsel{l: S} \proves \mapchar{\btsel{l: S}}{s} \hastype \Proc $.

		\item	If $U = \btbra{l_i: S_i}_{i \in I}$ then whenever
				$\forall i \in I, \Gamma; \es; \Delta' \cat s: S_i \proves \mapchar{S_i}{s} \hastype \Proc$ then
				$\es; \es; \set{t_i: \btout{S_i}}_{i \in I} \tinact \cat s: \btbra{l_i: S_i}_{i \in I} \proves \mapchar{\btbra{l_i: S_i}_{i \in I}}{s} \hastype \Proc $.

		\item	If $U = \vart{t}$ then
				$\Gamma \cat \varp{X_{\vart{t}}}: \set{s: S}; \es; s: S \proves \mapchar{t}{s} \hastype \Proc$

		\item	If $U = \trec{t}{S}$ then whenever $\exists \Gamma, \Delta$ such that
				$\Gamma \cat \varp{X_{\vart{t}}}: \set{s: \trec{t}{S}}; \es; \Delta \cat t: \btout{S'} \tinact \cat s: S \proves \mapchar{S}{s} \hastype \Proc$ then
				$\Gamma; \es; \Delta \cat t: \btout{S'} \tinact \cat s: S \proves \mapchar{\trec{t}{S}}{s} \hastype \Proc$.

		\item	If $U = \tinact$ then $\es; \es; \es; \proves \mapchar{\tinact}{s} \hastype \Proc$.
...
\begin{proof}[Sketch]
	The proof of part 1 is done by induction on the the syntax of $S$.
...
%					We do a sub-case analysis on recursive unfold. A notable case is the output prefix.
%					$S = \trec{t}{\btout{U} S'}$ with $\mapchar{\btout{U} S' \subst{\trec{t}{\btout{U} S'}}{\vart{t}} }{s} = \bout{t}{\omapchar{U}} \bout{t}{s} \inact $. 
%					\[
%						\tree{
%							\begin{array}{l}
%								\Gamma; \es; s: S' \subst{\trec{t}{\btout{U} S'}}{\vart{t}} \cat t: \btout{S' \subst{\trec{t}{\btout{U} S'}}{\vart{t}}} \tinact \hastype \bout{t}{s} \inact \hastype \Proc
%								\\
%								\Gamma; \es; \Delta \proves \omapchar{U} \hastype U  \qquad \text{(Induction)}
%							\end{array}
%						}{
%							\Gamma; \es; \Delta \cat s: \btout{U} S' \subst{\trec{t}{\btout{U} S'}}{\vart{t}} \cat t: \btout{S' \subst{\trec{t}{\btout{U} S'}}{\vart{t}}} \tinact \hastype \bout{s}{\omapchar{U}}\bout{t}{s} \inact \hastype \Proc
%						}
%					\]
...
\begin{example}[Type inhabitation of $\trec{t}{\btout{\tinact} \vart{t}}$]
	\[
		\tree{ 
			\tree{
				\begin{array}{l}
				\varp{X}_{\vart{t}}: \set {s : \trec{t}{\btout{\tinact} \vart{t}}}; \es; s : \trec{t}{\btout{\tinact} \vart{t}} \proves \mapchar{t}{s} = \varp{X}_{\vart{t}} \hastype \Proc
				\\
				\es: \es: s': \tinact \proves s' \hastype \tinact
				\end{array}
			}{
				\Gamma \cat \varp{X}_{\vart{t}}: \set {s : \trec{t}{\btout{\tinact} \vart{t}}}; \es; t: \btout{\trec{t}{\btout{\tinact} \vart{t}}} \tinact \cat s : \btout{\tinact} \trec{t}{\btout{\tinact} \vart{t}} \proves \mapchar{\btout{\tinact} \vart{t}}{s} = \bout{s}{s'} \bout{t}{s} \tinact \hastype \Proc
			}
		}{
				\Gamma \cat \varp{X}_{\vart{t}}: \set {s : \trec{t}{\btout{\tinact} \vart{t}}}; \es; t: \btout{\trec{t}{\btout{\tinact} \vart{t}}} \tinact \cat s : \btout{\tinact} \trec{t}{\btout{\tinact} \vart{t}} \proves \mapchar{\trec{t}{\btout{\tinact} \vart{t}}}{s} = \bout{s}{s'} \bout{t}{s} \tinact \hastype \Proc
		}
	\]
\end{example}
...
%									{\hby{\bactinp{t}{\map{\btinp{U} \tinact}^{x}}}}& & &
		...
		\item	Part 2 
				...
				\item The action comes from the interaction of $P$ and $\hotrigger{t}{x}{s}{V_1}$: This case is not possible, due to the freshness of $t$.
				\end{enumerate}
	\end{enumerate}
	\qed
\end{proof}
...
\begin{proof}
	\begin{enumerate}
		\item	The proof for Part 1 is done by contradiction.
				...
				\]
				with $\subj{\ell_2} = n_1$.
				But then in the latter result we can observe an action on $t'$ and on the latter
		...
		\item	Part 4. Let typed relation
				...
				\begin{itemize}
					\item	If $\subj{\ell_1} \not= n_1$ then the proof is straightforward from the premise of the proposition.
							...
							implies
							\[
								\horel{\Gamma}{\Delta_2}{\newsp{\widetilde{m_2}'}{Q \subst{m}{x}} \Par \bout{t}{n_2} \inact}
								{\Hby{\ell_2}}
								{\Delta_2'}{\newsp{\widetilde{m_2}'}{Q' \subst{m}{x}} \Par \bout{t}{n_2} \inact}
							\]
							and
							\[
								...
								{\hwb}
								{\Delta_2'}{\newsp{\widetilde{m_2}'}{Q' \subst{m}{x}} \Par \bout{t}{n_2} \inact \Par C_2}
							\]
					...
					\item	$\subj{\ell_1} = n_1$. The case is similar as the previous case.

					\item	$\ell_1 = \tau$. This implies the untyped transitions
							\begin{eqnarray*}
								\newsp{\widetilde{m_1}}{P \subst{n_1}{x}} &\hby{\ell_1'}& \newsp{\widetilde{m_{11}}}{P_1 \subst{n_1}{x}}\\
								\newsp{\widetilde{m_1}}{P \subst{n_1}{x}} &\hby{\ell_2'}& \newsp{\widetilde{m_{12}}}{P_2 \subst{n_1}{x}}\\
								\ell_1' &\asymp& \ell_2'
							\end{eqnarray*}
					\end{itemize}
