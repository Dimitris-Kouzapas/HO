...
%				whenever $\Gamma; \es; \Delta' \cat s: S \proves \mapchar{S}{s} \hastype \Proc$ then
				$\es; \es; t: \btout{S} \tinact \cat s: \btsel{l: S} \proves \mapchar{\btsel{l: S}}{s} \hastype \Proc $.

		\item	If $U = \btbra{l_i: S_i}_{i \in I}$ then
%				whenever $\forall i \in I, \Gamma; \es; \Delta' \cat s: S_i \proves \mapchar{S_i}{s} \hastype \Proc$ then
...
%				$\Gamma \cat \varp{X_{\vart{t}}}: \set{s: S}; \es; s: S \proves \mapchar{t}{s} \hastype \Proc$
		...
		\item	If $U = \trec{t}{S}$ then whenever $\exists, \Gamma, \Delta$ such that
				$\Gamma; \es; \Delta \cat t: S'' \cat s: S \subst{\tinact}{\vart{t}} \proves \mapchar{S \subst{\tinact}{\vart{t}}}{s} \hastype \Proc$
				then $\exists S'$
