...
\begin{lemma}[Trigger Process Equivalence]
	...
	\jasks{We also need to say that $V_1, V_2$ are of type $U$, right?}
	\begin{enumerate}[1)]
		\item	If
				\[
...
\begin{proof}
	...
	\jasks{I disagree, we should show both cases: this is the kind of claims that make reviewers upset :) Since both parts deal with different equivalences it is unclear how you could reuse cases of 2 into 1. I know it is a lot of work, but showing that the triggers are equivalence is really the key of our approach.}
	...
	\begin{enumerate}
		...
		\item	\jasks{I would probably put cases 2 and 3 within a single case, which is the case when the right-hand side process moves. }Case
			...
			\]
			%
				implies from the higher order bisimilarity requirement of the premise \jasks{(what premise?)} and
				the freshness of $t$ that $\exists Q', \Delta_2''$ such that
			%
				\begin{eqnarray}
					\begin{array}{crll}
		...
		%				\label{lem:wb_eq_wbf1}
				...
				\smallskip
						...
						%
							...
							\end{eqnarray}
						%
							and for fresh $t$ \jasks{(below we mean $\fwb$, right?)}:
								\nhorel{\Gamma}{\Delta_1'}{\newsp{\widetilde{m_1}}{P' \Par \fotrigger{t}{x}{s}{\btinp{U} \tinact}{V_1}}}
								{\hwb}
								{\Delta_2'}{\newsp{\widetilde{m_2}}{Q' \Par \fotrigger{t}{x}{s}{\btinp{U} \tinact}{V_2} }}
								{lem:fwb_sub_hbf3}
...
\begin{lemma}[Process Substitution]
	...
	\jasks{Don't we need to say that $x$ occurs free in in $P_i$? Also, we have $x$ both in $P$ and in the parameter of the abstraction, it is better to have different variables, just to avoid confusion.}
	If
	\[
...
\begin{proof}
	...
	\begin{enumerate}
		\item	Case $P_1 \not\scong Q \Par \appl{x}{n}$ \jasks{(I would add: that is, the substitution does not affect top-level processes.)}
				...
				\end{eqnarray*}
				to further conclude \jasks{(We need further explanation here. The idea is that since $\ell_1, \ell_2$ do not pertain to the substitution, so we can consider any $\abs{x}{R}$ instead of $\trvalx{t}$, right?)}
				\begin{eqnarray}
		...
		\jasks{That is, $P_1$ has exactly one occurrence of $x$ at top-level. But we should explain why these two cases exclude other (non linear) possibilities, such as, e.g., $Q' \Par \appl{x}{n_1} \Par \appl{x}{n_2}$.} 
				...
				\begin{itemize}
					...
					\item	Sub-case \jasks{(I am not sure if I follow: is this case just the (deterministic) application of $n_1$?)}
							...
							\end{eqnarray}
							%
							From the requirements of $\Re$ we conclude that \jasks{(General comment: I notice that often you write `we conclude' but you mean `we infer' or `we obtain'. One ``concludes'' only at the very end.)}
							\begin{eqnarray*}
							...
							\end{eqnarray*}
							%
							From \lemref{lem:tigger_subst} we can deduce that $\forall \abs{x}{R}$, if $\exists \Delta_5, \Delta_6$ then
							\begin{eqnarray*}
							...
							%
								\nhorel{\Gamma}{\Delta_3}{\newsp{\widetilde{m_1}}{Q \subst{(\abs{x}{R})}{x} \Par \appl{(\abs{x}{R})}{n_1}}}
								{\ \Re\ }
								{\Delta_4}{\newsp{\widetilde{m_2}}{P_2'' \subst{(\abs{x}{R})}{x} \Par \appl{(\abs{x}{R})}{n_2}}}
								{lem:proc_subst4}
							%	
