...
\begin{definition}[Session Type Prefix Deletion]
	Given a session type $S$, the set $\set{S}$ is defined inductively as follows:
	\[
	\begin{array}{c}
...
\begin{proof}[Sketch] The proof proceeds by mutual induction on the syntax of types.
	\begin{enumerate}
		\item	The proof the results from Parts 2 and 3.
				We do a case analysis on the syntax of $U$.
			\begin{enumerate}
...
The proof is straightforward from Rules $\trule{Sess}$ and $\trule{Sh}$ (cf. \figref{fig:typerulesmy}).
			...
			\end{enumerate}
		\item	The proof is by induction on the the syntax of $S$.
			...
			\begin{enumerate}
				...
				\item	Case $S = \btinp{S_1} S_2$. Then, by \defref{def:char}, we have 
					...
					\item	Case $S = \trec{t}{S'}$. Then, by \defref{def:char}, $\mapchar{S} = \mapchar{S' \subst{\tinact}{\vart{t}} }{u}$.
							The proof is done by induction on the shape of $S'$; we detail a few sub-cases.
							\begin{enumerate}
		...
		\item	The proof  uses the result of Part 1.
			...
			\begin{enumerate}
				...
				\item	$U = \chtype{S}$. Similar argumentation as in the previous case.

				\item
						$U = \lhot{U'}$ then from Part 1 we get that
						$\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'$.
...
the trigger processes $\htrigger{t}{V}$ (cf.~\eqref{eqb:0}) and $\ntrigger{t}{V}$.
...
\begin{itemize}
	...
	\begin{enumerate}
		\item	First direction. Consider the typed relation (we omit the type information for readability):
				\begin{eqnarray*}
					\Re &=& \set{	\newsp{\widetilde{m_1}}{P}\ , \ 
				...
				\[
					...
					{\Delta_1'}{\newsp{\widetilde{m_1}'}{P' \Par \newsp{s}{\bout{t}{s} \inact}  } }
				\]
				which implies from the freshness of $t$ that there exists a $Q'$ such that
				\begin{eqnarray}
					\horel{\Gamma}{\Delta_2}{\newsp{\widetilde{m_2}}{Q \Par \newsp{s}{\bout{t}{s} \inact}  } }
					{\Hby{\ell_2}}
					{\Delta_2'}{\newsp{\widetilde{m_2}'}{Q' \Par \newsp{s}{\bout{t}{s} \inact}  } }
		...
		\item	Second direction. Consider the typed relation (we omit the type information for readability)
				\begin{eqnarray*}
					\Re &=& \set{	\newsp{\widetilde{m_1}}{P \Par \newsp{s}{\bout{t}{s} \inact}}\ , \ 
				...
				\begin{enumerate}
					...
					\[
							...
							\[
								...
								{\Delta_2'}{\newsp{\widetilde{m_2}}{Q}}
							\]
							\newc{JP: Above we should have $Q'$ instead of $Q$ in the RS, right?}
							...
							\[
								...
								{\Delta_4}{\news{\widetilde{m_2}}{Q  } }
							\]
					\item	The interaction of name $t$ with process $P$: this cannot occur 
	...
	\item Part 3 relies on Part 1. We do an analysis on the two
	directions of the if and only if requirement.
	\begin{enumerate}
		\item First direction. Let $\Re$ be the typed relation (for readability, we omit the type information):
				\begin{eqnarray*}
					\Re &=&	\set{	\newsp{\widetilde{m_1}}{P \Par \htrigger{t}{V_1}}\ ,\ 
				...
				\begin{enumerate}
					\item $P$ moves on its own:
							...
							\]
							The proof is similar to Case 1 of Part 1 of this lemma.
	...
	\horel{\Gamma}{\Delta_2}{\newsp{\widetilde{m_2}}{Q \Par \ntrigger{t}{V_2}  } }
		...
		\Re &=&	\set{	\newsp{\widetilde{m_1}}{P \Par \ntrigger{t}{V_1}}\ ,\ 
				...
				other cases are similar to previous cases.
				\begin{enumerate}
...
%									{\hby{\bactinp{t}{\map{\btinp{U} \tinact}^{x}}}}& & &
		...
		\item	Part 2.
				...
				\newc{JP: But the equivalence of triggers is up-to $\hwb$ not $\fwb$...}
				The result is then a consequence of \lemref{lem:alt_tr} (Part 3).
...
\begin{lemma}[Trigger Process Application]
	...
	\begin{enumerate}
		\item
			...
			\]
			then $n_1, n_2$ sessions \newc{JP: what does this mean?} and $\dual{n_1} \in \fn{P}$ and $\dual{n_2} \in \fn{Q}$.
...
\begin{proof}
	...
	\begin{eqnarray*}
		\Re	&=&	\set{
			...
			&&		}
	\end{eqnarray*}
	We show that $\Re$ is a higher-order bisimilation. Suppose that 
	%
	\begin{eqnarray}
		\horel{\Gamma}{\Delta_1'}{\newsp{\widetilde{m_1}}{P_1 \subst{{\abs{x}{R}}}{z} }}
		{\hby{\ell_1}}
...
%					\horel{\Gamma}{\Delta_1'}{P_2 \subst{\abs{\widetilde{x}}{\mapchar{U}{\widetilde{x}}}}{x}}{\hwb}{\Delta_2'}{Q_2 \subst{\abs{\widetilde{x}}{\mapchar{U}{\widetilde{x}}}}{x}}\\
		...
		\item	Case $\ell = \news{\widetilde{m_1}'} \bactout{n}{\abs{\widetilde{x}}{P}}$:
				...
				\noi From transition \eqref{lem:wb_is_wbc1} we imply:
			%
			\[
			...
			\]
			%
			From \lemref{lem:alt_tr} we get that
			%
			\[
			...
			\[
				...
				{\Delta_2''}{}{\newsp{\widetilde{m_1}'}{Q_2' \Par \newsp{s}{\appl{(\abs{y}{\binp{t'}{x} (\appl{x}{y})) }{s}}  \Par \bout{\dual{s}}{\abs{\widetilde{x}}{Q}} \inact } }}
			\]
				\noi From the  second case of this proof we can conclude that $\forall R$ with $\fpv{R} = x$:
			%
			\[
			...
			\]
				\noi with simple determinate transitions is easy to see that $\forall R$ with $\fpv{R} = x$:
			%
			\[
