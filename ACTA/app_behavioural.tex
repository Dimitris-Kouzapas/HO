...
\begin{definition}[Session Type Unfolding] \newc{Given a session type $S$, the function $\unfold{S}$ is defined as:}
	\[
	\begin{array}{c}
...
\begin{definition}[Session Type Prefix Deletion] \newc{Given a session type $S$, the set $\set{S}$ is defined inductively as follows:}
	\[
	\begin{array}{c}
...
%						$\Gamma; \es; \Delta \cat \set{t_i: \btout{S_i} \tinact}_{i \in I} \cat s: \trec{t}{S'} \proves \mapchar{\trec{t}{S'}}{s} \hastype \Proc$.
		...
		\begin{enumerate}
			...
			\item	
\NY{Is this correct? Better typing?} If $U = \lhot{U'}$ and   $\Gamma; \es; \Delta \proves \omapchar{U'} \hastype U'$ then \\
...
\begin{proof}[Sketch]
	...
	\end{itemize}
	The proof for parts 2 and 3 is similar. \newc{NO, we need to add some cases for these other parts.}
	\qed
\end{proof}
