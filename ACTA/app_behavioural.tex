...
We thesis follows by noticing that, in the first case, $P'$ can match the move from $P$; the second case cannot occur because of the definition of deterministic transitions. 
...
First we introduce notation $\mact{\ell}$, used throughout
...
\begin{definition}
	\[
		...
		\left\{
		\begin{array}{lcl}
			\newsp{\widetilde{m_2}}{\bactout{n}{V_2}} & \quad & \text{for some } V_2, \widetilde{m_2} \text{ if } \ell = \newsp{\widetilde{m_1}}{\bactout{n}{V_1}}
			\\
			\ell &&  \text{otherwise}
		\end{array}
		\right.
	...
	\begin{enumerate}[a)]
		\item	First direction. Consider the typed relation (we omit the type information):
				...
				\[
					...
					{\Delta_1'}{\newsp{\widetilde{m_1}'}{P' \Par \newsp{s}{\bout{t}{s} \inact}  } }
				\]
				which implies from the freshness of $t$ that there exists $Q'$,
...
%				\JP{ We need to say that $\ell_2$ is only different from $\ell_1$ in the output case!}
				...
				\]
				with $C_1 = \htrigger{t'}{V_1}$ and $C_2 = \htrigger{t'}{V_1}$ (if $\ell$ and $\mact{\ell}$ are output actions
...
%				\JP{Why? I think $\ell_1$ can come from $P$ and be different from $n_1$..}
		...
		\item	Part 4. Let $\Re$ be the typed relation
				...
				\[
					...
					{\Delta_1'}{\newsp{\widetilde{m_1}'}{P' \subst{n_1}{x}}}
				\]
				We prove that $\Re$ is a higher-order bisimulation by a case analysis on the subject of action $\ell$.
				\begin{itemize}
...
\begin{proof}
	...
	\begin{enumerate}
		\item	Case $P_1 \not\scong Q \Par \appl{z}{n}$ that is,
				...
				In other words we can imply from the freshness of $t$ that $\subj{\ell} \not= t$.
	%			\jasks{(This sentence on freshness seems misplaced - it makes sense only below.)}
				...
				\begin{eqnarray*}
					...
					{\Delta_1''}{\newsp{\widetilde{m_1}}{P_1' \subst{\trvalx{t}}{z} }}
				\end{eqnarray*}
				which, in turn, implies that there exist $\Delta_2'', P_2'$ such that
				\begin{eqnarray}
					\horel{\Gamma}{\Delta_2}{\newsp{\widetilde{m_2}'}{P_2 \subst{\trvalx{t}}{z} }}
					{\Hby{\mact{\ell}}}
					{\Delta_2''}{\newsp{\widetilde{m_2}'}{P_2' \subst{\trvalx{t}}{z} }}
...
%				\jasks{There is a problem with equation labels above!}
		...
		\item	Case $P_1 \scong P \Par \prod_{i \in I} \appl{z}{n_i} \Par \appl{z}{n_1}$ such that
				$P \not= P' \Par \appl{z}{n'}$. This is the case where action $\ell$ might
