...
several lemmas:
\begin{enumerate}[$-$]
\item \lemref{l:altrig} connects characteristic and higher-order trigger processes with an alternative trigger process
...
\textcolor{blue}{JP - This paragraph is not very clear:}
The alternative trigger process has a simpler formulation
from the higher-order trigger process (cf.~\eqref{eqb:0}) which is useful to proofs.
However, the input of characteristic values on name $t$
results in the creation of redundant parallel components
of the form $\news{s}{\bout{t}{s}}$ which need to
be treated in a bisimulation closure. The next
...
%							The proof is a consequence of Part 3 \lemref{lem:trigger_application}.
							...
							\end{eqnarray}
							for some $\Delta_4'$.
							From part 3 of \lemref{lem:trigger_application} we obtain
%							\[
%								\mhorel{\Gamma}{\Delta_3}{  \newsp{\widetilde{m_1}'}{P \Par \prod_{i \in I\backslash{\set{k}}} \binp{t_i}{x} (\appl{x}{n_i})
%								\Par  \mapchar{U}{x} \subst{n_k}{x} }}
%								{ \hby{\tau} }
...
%							\begin{eqnarray}
%							\mhorel{\Gamma}{\Delta_5}{  \newsp{\widetilde{m_1}''}{P' \Par R' \subst{\tilde{n}}{\tilde{x}}}}
%							{ \hwb }
