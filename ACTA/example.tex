...
\begin{example}\label{exam:jr}
	\dkmargin{different definition of characteristic process - fixed, please check}
	...
	We %compare our approach to that in~\cite{JeffreyR05} by contrasting 
	contrast 
	the transitions from $P$. 
	In our framework, we have a typed transition
	$
    ...
    \figref{f:comparison} presents a complete comparison of the labelled transitions in our approach (\figref{tablejr:1}) and in~\cite{JeffreyR05} (\figref{tablejr:2}).
    In our approach, we let 
$$
%	\mapchar{\btinp{U} \inact}{s}  =  \binp{s}{x} \appl{x}{(\abs{y}{(\appl{y}{a}))}}\qquad \textrm{for some fresh $a$}
	\mapchar{U}{x}  =  \appl{x}{(\abs{y}{(\appl{y}{a}))}}\qquad \textrm{for some fresh $a$}
$$
	...
	(three, see Lines (1), (2), and (3) in \figref{tablejr:2}) and at the end, two replicated processes remain (on $t$ and $k$).
	This is how linearity information in session types enables simpler bisimulations.
