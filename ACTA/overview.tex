...
\begin{enumerate}[(a)]
	\item We replace $(\star)$ with a clause involving a process closure \emph{more tractable} than $R\subst{V}{x}$; and 
...
refined
rule 
for  \jpc{(higher-order)} input admits only names, trigger values, and characteristic values:
%\[
%\tree {
