...
%\end{equation}
%\noi 
In fact, \newc{through a name application and a synchronization on session endpoint $s$}
...
\newc{To illustrate the key idea underlying characteristic processes, consider}
$S = \btinp{\shot{S_1}} \btout{S_2} \tinact$, i.e.,  a session type denoting a protocol that 
 first
...
$\omapchar{U}$ denotes its \emph{characteristic value} (cf.~\defref{def:char}).
...
\dk{The lack of matching is compensated here with the use of (session) types.}
