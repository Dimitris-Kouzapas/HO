...
a simple term inhabiting that type (\defref{def:char}).
%Thus, characteristic processes follow the communication structures decreed by session types.

\dk{As an example of a characteristic process consider}
...
\dk{Then, a process $\binp{u}{x} (\bout{u}{s_2} \inact \Par \appl{x}{s_1})$
that inhabits $S$ is a kind of characteristic process for $S$ along
name $u$. In fact, for the needs of the theory developed in this paper
we consider characteristic processes that contain more information (cf.~\defref{def:char}).
For type $S$ the characteristic process we consider has the form
$\binp{u}{x} (\bout{t}{u} \inact \Par \appl{x}{s_1})$ for fresh name $t$.}
...
%Using this rule, 
we define an alternative, refined  LTS on typed processes, denoted  $\hby{\ell}$.
...
trigger processes
\jpc{(cf.~\eqref{eq:4})}.
The key of this alternative LTS is 
...
\dk{The lack of matching is compensated with the use of types.
Matching gives the ability to the observer of the
to test a received name. Instead, in our theory a process trigger
embeds a name into a characteristic process so to observe its session behaviour.
Thus, equivalent processes deal with (possibly different) names that have
the same behaviour.}
