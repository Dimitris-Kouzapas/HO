...
suggested in Definition~\ref{def:ep}(2). We state the
requirements below.
...
\begin{proposition}
	...
	If $\mapa{\tau} = \tau$ then
	the requirements of Definition~\ref{app:def:opc_strong} imply
	the requirements of Definition~\ref{def:ep}(2).
\end{proposition}

\begin{proof}
	The proof is trivial by substituting the $\tau$ action
	on labels $\ell_1$ and $\ell_2$ in Definition~\ref{app:def:opc_strong}
	to get Definition~\ref{def:ep}(2).
\end{proof}
...
$\enco{\pmapp{\cdot}{1}{f}, \tmap{\cdot}{1}, \mapa{\cdot}^{1}}: \HOp \to \HO$ is precise.
A precise encoding requires to prove three independent results:
\begin{itemize}
	\item	Type preservation, stated in Lemma~\ref{app:prop:typepres_HOp_to_HO}.
	\item	Operational Correspondence, stated in Lemma~\ref{app:prop:op_corr_HOp_to_HO}.
		Note that we prove a stronger operational correspondence condition,
		as in Definition~\ref{app:def:opc_strong},
		than the condition suggested in Definition~\ref{def:ep}(2).
	\item	Full Abstraction, stated in Lemma~\ref{app:prop:fulla_HOp_to_HO}.
\end{itemize}
...
$\hby{\ell}$:
\begin{definition}[$\mapa{\cdot}^{1}: \mathcal{A} \to \mathcal{A}$]
...
in Definition~\ref{app:def:opc_strong}.
