...
higher-order and characteristic bisimulations) which only require observation of finite %number of 
higher-order values,  
...
languages. 
In these works, typed properties and linearity
play a fundamental role in the analysis. In general, linear and
session types are suitable to encode ``sequentiality'' in the sense of 
...
to encode \HOp into \sessp (\defref{def:enc:HOp_to_p}).
...
%with itself.
%This comparison, detailed in \appref{app:jandr}, reveals that our approach 
...
%%The main differences of the triggeRnamred
