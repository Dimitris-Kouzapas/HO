...
In turn, the linear/shared distinction is central in proper definitions of trigger processes, which are essential to encodings (cf.~\defref{def:enc:HOp_to_FO}) and behavioral equivalences (cf. \defref{def:XX}).
...
is also used in this work for the encoding of \HOp into \sessp (\defref{def:enc:HOp_to_FO}).
...
\begin{enumerate}[i)]
	\item	The output of a higher-order value $\abs{x}{Q}$ on name
		...
		(cf.~characteristic bisimulation \defref{def:FO_bisim}),
