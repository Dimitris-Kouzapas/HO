...
Early expressiveness studies for higher-order calculi are~\cite{Tho90,San923,San96int}; 
...
The work~\cite{San923} given an encoding of the higher-order $\pi$-calculus
into the first-order $\pi$-calculus which is fully abstract swith respect to barbed congruence. 
...
With respect to these previous results, our work demonstrates that using a type structure based on sessions
...
More importantly, we showed that $\HO$, a  minimal higher-order session calculus (no name passing, only first-order application) suffices to encode $\sessp$ (the session calculus with name passing) but also 
$\HOp$  and 
its extension of $\HOp$ with higher-order applications ($\HOpp$). 
