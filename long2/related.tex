...
In \cite{DBLP:journals/iandc/PerezCPT14} typed behavioral equivalencies for binary session types are studied for 
a session $\pi$-calculus without shared names. Issues of strong normalization, confluence, $\tau$-inertness are covered.
...
The theory for higher-order session types
differentiates from 
the work in~\cite{dkphdthesis,DBLP:conf/forte/KouzapasYH11,KY13}
that studies the behavioural semantics for first-order
...
theory we are motivate and in fact we continue the line drawn
originally by Sangiorgi~\cite{San96H,SangiorgiD:expmpa},
and by Jeffrey and Rathke~\cite{DBLP:journals/lmcs/JeffreyR05}.
Sangiorgi as part of his Ph.D.~research~\cite{San96H,SangiorgiD:expmpa}
...
triggered bisimulation, also defined on first-order labels
...
Rathke to introduce their own version of triggered
bisimulation~\cite{DBLP:journals/lmcs/JeffreyR05}.
%for a full higher-order $\pi$-calculus that allows
%higher-order applications.
Like Sangiorgi the labelled transition semantics
that observe first-order triggered values instead of
higher-order values.
...
$\abs{z}\btinp{t}{x} \appl{x}{z}$ as an input instead of the
