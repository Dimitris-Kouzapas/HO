...
%
\begin{definition}[Extended Characteristic Process]\rm
...
%
	...
	\begin{array}{rclcl}
		...
		\\
		\map{\btinp{\lhot{L}} S}^{u} &=& \binp{u}{x} (\map{S}^u \Par \appl{x}{\map{L}^y}) %& & %n \textrm{ fresh}
		\\
		\map{\btout{\shot{L}} S}^{u} &=& \bout{u}{\map{L}^x} \map{S}^u
		\\
		\map{\btinp{\shot{L}} S}^{u} &=& \binp{u}{x} (\map{S}^u \Par \appl{x}{\map{L}^y}) %& & n \textrm{ fresh}
		\\
		\\
