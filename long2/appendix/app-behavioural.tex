...
\subsection{Proof for \thmref{the:coincidence}}
\label{app:sub_coinc}

We split \thmref{the:coincidence} into 
...
%
	\[
		\begin{array}{crll}
			...
			\hby{\btau}
			&\Delta_2''& \proves & \newsp{\tilde{m_2'}}{Q_1' \Par R \subst{\tilde{x}}{\tilde{m}}}%\appl{x}{\tilde{m}} \subst{\abs{\tilde{x}}{R'}}{x}}
		\end{array}
	\]
%
%	\[
...
%
	...
	\]
%
	\noi From \lemref{lem:subst_equiv} we get that $\forall R$ with $\fv{R} = x$
%
	\begin{eqnarray*}
...
\]
%
	\noi To conlude from \lemref{lem:proc_subst} that
...
%
	...
	\end{eqnarray}
%
	From~\ref{lem:wbc_is_cong14} and the Substitution Lemma~\lemref{lem:subst} we get that
	\[
