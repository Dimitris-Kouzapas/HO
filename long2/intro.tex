...
from typed and untyped perspectives (see, e.g.,~\cite{ThomsenB:plachoasgcfhop,SangiorgiD:expmpa,San96int,DBLP:journals/lmcs/JeffreyR05,mostrous_phd,DBLP:journals/iandc/LanesePSS11,DBLP:conf/icalp/LanesePSS10,DBLP:conf/esop/KoutavasH11,XuActa2012}).
...
Encoding of a general recursive process $\recp{X}{P}$ is another challenge, since 
...
%	\item \S\,\ref{sec:overview} overviews key ideas of our tractable bisimulations.
	...
	\item	\secref{sec:positive} %and \S\,\ref{sec:negative}
		...
		These encodings 
		are shown to be \emph{precise} (\propref{prop:prec:HOp_to_HO} and \propref{f:enc:hotopi}).
	...
	\item	\secref{sec:extension} studies extensions of \HOp. We show that 
		...
		(\thmref{f:enc:hopiptohopi} and \ref{f:enc:phopiptohopi}).
