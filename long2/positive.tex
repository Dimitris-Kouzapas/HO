...
%
\begin{definition}[Encoding \HOp into \HO]\rm
...
{\em recursion} ($\pmapp{\recp{X}{P}}{1}{f}$ and $\pmapp{z_\varp{X}}{1}{f}$). 
...
\NY{i) record a mapping from recursive variable $X$ to process variables $x$;
...
\begin{proposition}[Type Preservation, \HOp into \HO]\rm
...
\begin{proposition}[Full Abstraction, \HOp into \HO]\rm
	...
	if and only if
	$\horel{\tmap{\Gamma}{1}}{\tmap{\Delta_1}{1}}{\pmapp{P_1}{1}{f}}{\wb}{\tmap{\Delta_2}{1}}{\pmapp{Q_1}{1}{f}}$.
\end{proposition}
...
\begin{proposition}[Precise encoding of \HOp into \HO]\rm
...
%\begin{proposition}[Order Preservation]\rm
...
\begin{definition}[Encoding \HOp to \sessp]\rm
	...
	in d\figref{fig:enc:HOp_to_p}.
\end{definition}
%

\input{figures/fig-HOp_to_p}
			...
			\item
				...
				\begin{enumerate}[-]
					\item 
						...
						\hby{\mapa{\ell_1}^{2}}
						\Gamma' \cdot \tmap{\Gamma}{2};\, \tmap{\Delta'}{2} \proves \pmap{P'}{2} \Par \repl{} \binp{a}{y} \binp{y}{x} \pmap{Q}{3}$
					\item 
						$\tmap{\Gamma}{2};\, \tmap{\Delta}{2} \proves \pmap{P}{2} 
						\hby{\mapa{\ell_1}^{2}}
						\tmap{\Gamma}{2};\, \Delta'' \proves \pmap{P'}{2} \Par \binp{s}{y} \binp{y}{x} \pmap{Q}{3}$
				\end{enumerate}
%			\item	 
			...
			\begin{enumerate}[a)]
				...
				\item   %% sinput
					...
					\item	$\exists Q, P'$ such that
						...
						and $\horel{\mapt{\Gamma}^{2}}{\mapt{\Delta'}^{2}}{\map{P'}^{2}}{\wb}{\mapt{\Delta'}^{2}}{\news{a}(R \Par \repl{} \binp{a}{y} \binp{y}{x} \pmap{Q}{3})}$
					\item	$\exists Q, P'$ such that
						$P \hby{\bactinp{n}{\abs{x}Q}} P'$
						and $\horel{\mapt{\Gamma}^{2}}{\mapt{\Delta'}^{2}}{\map{P'}^{2}}{\wb}{\mapt{\Delta'}^{2}}{\news{s}(R \Par \binp{s}{y} \binp{y}{x} \pmap{Q}{3})}$  
					\end{enumerate}
...
%			   or $R_a = \binp{a}{y} \binp{y}{x} \pmap{Q}{3}$.
		\item	 
			...
			$\mapt{\Gamma}^{2};\, \mapt{\Delta}^{2} \proves  \map{P}^{2} 
			\hby{\mapa{\ell_1}^{2}}
			\Gamma' \cdot \mapt{\Gamma}^{2};\, \mapt{\Delta'}^{2} \proves \map{P'}^{2} \Par \repl{} \binp{a}{y} \binp{y}{x} \pmap{Q}{3}$
			\item 
			$\mapt{\Gamma}^{2};\, \mapt{\Delta}^{2} \proves  \map{P}^{2} 
			\hby{\mapa{\ell_1}^{2}}
			\mapt{\Gamma}^{2};\, \Delta'' \proves \map{P'}^{2} \Par   \binp{s}{y} \binp{y}{x} \pmap{Q}{3}$
			\end{enumerate}
%			\item	 
%			   If  $\stytra{\Gamma}{\bactinp{k}{\abs{x}Q}}{\Delta}{P}{\Delta'}{P'\subst{\abs{x}Q}{X}}$
%			   then 
%			   $\map{P}^{3} \hby{ \bactinp{k}{a} \map{P'}^{3}\subst{a}{x}$.				
		...
		\item	 %cinput
			...
			$\mapt{\Gamma}^{2};\, \mapt{\Delta}^{2} \proves \map{P}^{2} 
			\hby{\mapa{\ell_1}^{2}}
			\Gamma';\, \mapt{\Delta'}^{2} \proves  R$, for some $ \Gamma'$
			and \\ 
			$\horel{\mapt{\Gamma}^{2}}{\mapt{\Delta'}^{2}}{\map{P'}^{2}}{\wb}{\mapt{\Delta'}^{2}}{\news{a}(R \Par \repl{} \binp{a}{y} \binp{y}{x} \pmap{Q}{3})}$  			
			
			\item 
			$\mapt{\Gamma}^{2};\, \mapt{\Delta}^{2} \proves \map{P}^{2} 
			\hby{\mapa{\ell_1}^{2}}
			\mapt{\Gamma}^{2};\, \mapt{\Delta'}^{2} \proves  R$, 
			and \\ 
			$\horel{\mapt{\Gamma}^{2}}{\mapt{\Delta'}^{2}}{\map{P'}^{2}}{\wb}{\mapt{\Delta'}^{2}}{\news{s}(R \Par \binp{s}{y} \binp{y}{x} \pmap{Q}{3})}$  		
...
%			    and $\ell_2 = \mapa{\ell_1}^{3}$.
		...
		\item %% soutput
			...
			either 
			\begin{enumerate}[-]
			\item $R = \map{P'}^{2} \Par \repl{} \binp{a}{y} \binp{y}{x} \pmap{Q}{2}$
			\item $R = \map{P'}^{2} \Par \binp{s}{y} \binp{y}{x} \pmap{Q}{2}$
			\end{enumerate}
...
\begin{proof}
\iftodo
	\dk{Revise the case and put proof.}
\else
	Follows directly from \defref{def:enc:HOp_to_p}, using the substitution lemma (\lemref{lem:subst})
	and \propref{prop:typepres_HOp_to_p}.	
\fi
%	\dk{revise the cases and put proof}

	\qed
\end{proof}
\else
{\begin{proof} Mechanical using \lemref{lem:subst}. \qed 
\end{proof}
\fi
...
\begin{proposition}[Full Abstraction. From \HOp to \HO]\rm
	\label{prop:fulla_HOp_to_p}
	Let $P_1, Q_1$ be \HOp processes.
	$\horel{\Gamma}{\Delta_1}{P_1}{\wb}{\Delta_2}{Q_1}$
	if and only if
	$\horel{\tmap{\Gamma}{2}}{\tmap{\Delta_1}{2}}{\pmap{P_1}{2}}{\wb}{\tmap{\Delta_2}{2}}{\pmap{Q_1}{2}}$.
\end{proposition}
...
\iftodo
	\dk{Details of the proof can be found in~\propref{app:prop:fulla_HOp_to_p}
	(page~\pageref{app:prop:fulla_HOp_to_p}).}
\else
...
\begin{proposition}[Precise encoding of \HOp into \sessp]\rm
