...
Furthermore, they use the Higher-Order Bisimulation
(cf.~\ref{def:HO_bisim}) except in the case
of the \sessp sub-calculus that uses the
First-Order Bisimulation (cf.~\ref{def:FO_bisim})
...
%				(iii)~$\horel{\mapt{\Gamma}}{\mapt{\Delta'_1}}{\map{P'}}{\wb_2}{\mapt{\Delta'_1}}{Q}$.
		\end{enumerate}
...
\begin{definition}%\label{d:trabs}
	...
	Define $\auxmapp{\cdot}{\mathsf{v}}{\sigma}: \HOp \to \HOp$  as in Fig.~\ref{fig:auxmap}.
%
\begin{figure}[t]
\[
	\begin{array}{rcl}
...
which is defined as $\abs{x_1, \cdots, x_n}{\auxmapp{P}{\mathsf{v}}{\es} }$,
...
\begin{proof}
	\noi The proof is an easy induction on the map $\auxmapp{P}{\mathsf{v}}{\es}$.
	...
	$\appl{x}{\tilde{n}} \subst{\abs{\tilde{x}}{\bout{x_n}{y_m} \auxmapp{P}{\mathsf{v}}{\es}}}{x} \scong (\bout{x_n}{y_m} P) \subst{\tilde{x}}{\tilde{n}}$
...
\[
	\begin{array}{rcll}
		...
		\bout{\dual{s}}{\abs{\vmap{\tilde{n}}, z } \,{\binp{z}{x} \auxmapp{\pmapp{P}{1}{{f,\{\varp{X}\to \tilde{n}\}}}}{\mathsf{v}}{\es}}} \inact} &
		\quad \tilde{n} = \ofn{P}
		\\
...
\[
	\begin{array}{c}
		\begin{array}{rcl}
			...
			\\
			\dk{\tmap{\bbtout{\chtype{S_1}} S}{1}}	&\defeq&	\bbtout{\shot{\btinp{\shot{\chtype{\tmap{S_1}{1}}}}\tinact}} \tmap{S}{1}
			\\
			\dk{\tmap{\bbtinp{\chtype{S_1}} S}{1}}	&\defeq&	\bbtinp{\shot{\btinp{\shot{\chtype{\tmap{S_1}{1}}}}\tinact}} \tmap{S}{1}
			\\
...
\begin{remark}
	...
	\begin{eqnarray*}
		\mapt{\Gamma \cat k:\chtype{S}}^{1} & =  & \mapt{\Gamma}^{1} \cat k:\mapt{\chtype{S}}^{1} \\
		\tmap{\Gamma \cat \rvar{X}:\Delta}{1} & = & \tmap{\Gamma}{1} \cat X:\shot{(\tilde{S}_{\Delta}\,,\,S^*)}\qquad 
		\text{where }
%		S^* = \trec{t}{\big((\tilde{S}_{\Delta}\,,\, \btinp{\vart{t}}\tinact)\big)}
		S^* = \trec{t}{\btinp{\shot{(\tilde{S}_{\Delta}\,,\,t)}} \tinact}
	\end{eqnarray*}
\end{remark}
...
\begin{proposition}[Operational Correspondence for Recursive Processes]\rm
	...
	processes $R_1$, $R_2$, context $\C$, action $\ell'$,
	and mapping $f_1$ such that: 
	(1)~$\pmapp{P}{1}{f} \hby{\tau} \map{P'}^{1}_{f_{1}} \subst{\context{\C}{\map{P'}^{1}_{f_{1}}}}{X} = R_1$; and
	(2)~$R_1 \Hby{\ell'} R_2$, with $\ell' = \mapa{\ell}^{1}$.
\end{proposition}

\begin{proof}[Sketch]
	Part (1) follows directly from the definition of typed encoding for processes $\pmapp{\cdot}{1}{f}$
...
\begin{proposition}[Operational Correspondence, \HOp into \HO]
	...
	\begin{enumerate}[1.]
		\item
			If $\stytra{\Gamma}{\ell_1}{\Delta}{P}{\Delta'}{P'}$
			then $\exists \ell_2$ s.t. \\
			$\wtytra{\mapt{\Gamma}^{1}}{\ell_2}{\mapt{\Delta}^{1}}{\pmapp{P}{1}{f}}{\mapt{\Delta'}^{1}}{\pmapp{P'}{1}{f}}$
...
\begin{example}[Encode $\recp{X}{\bout{a}{m} \rvar{X}}$ into \HO]

We encode the \HOp process $\recp{X}{\bout{a}{m} \rvar{X}}$
into a \HO process. The first application of the mapping
will give:
%
\begin{eqnarray*}
	\pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f} &=&
	\newsp{s_1}{ \binp{s_1}{X} \pmapp{\bout{a}{m} \varp{X}}{1}{f, \rvar{X} \rightarrow x_ax_m} \Par\\
	&&\bout{\dual{s_1}}{ \abs{x_a x_m z} \binp{z}{X} \auxmapp{\pmapp{\bout{a}{m} \rvar{X}}{1}{f, \rvar{X} \rightarrow x_ax_m}}{v}{\es} } \inact}
\end{eqnarray*}
%
\noi with $\pmapp{\bout{a}{m} \rvar{X}}{1}{f, \rvar{X} \rightarrow x_ax_m}$ defined as:
%
\begin{eqnarray*}
	\pmapp{\bout{a}{m} \rvar{X}}{1}{f, \rvar{X} \rightarrow x_ax_m} &=&
	\bout{a}{\abs{x}{\binp{x}{X} \appl{X}{m}}} \pmapp{\rvar{X}}{1}{f, \rvar{X} \rightarrow x_ax_m}
	\\
...
%
\begin{eqnarray*}
	\auxmapp{\pmapp{\bout{a}{m} \rvar{X}}{1}{f, \rvar{X} \rightarrow x_ax_m}}{v}{\es} &=&
...
\begin{array}{l}
	...
	\pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f} \scong \\
...
\]
%
\noi We can observe $\pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f}$ as:
\[
	\begin{array}{cl}
		& \pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f}\\
		\scong&
		...
		\bout{a}{\abs{x}{\binp{x}{X} \appl{X}{m}}}\\
		 & \newsp{s_1}{\bout{\dual{s_1}}{V} \inact \Par \binp{s_1}{X} \bout{a}{\abs{x}{\binp{x}{X} \appl{X}{m}}} \newsp{s_2}{\bout{\dual{s_2}}{\abs{x_a x_m z}{\appl{X}{x_a x_m z}}} \inact} \Par \appl{X}{a,m, s_2}}\\
		\scong& \bout{a}{\abs{x}{\binp{x}{X} \appl{X}{m}}} \pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f}
		\\
		\by{\bactout{a}{\abs{x}{\binp{x}{X} \appl{X}{m}}}}& \pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f}
	\end{array}
\]
%
\noi We further show that $\pmapp{\recp{X}{\bout{a}{m} \rvar{X}}}{1}{f}$ is typable:
\begin{eqnarray}
	\tree{
		\begin{array}{l}
...
\label{def:enc:HOp_to_FO}
	Define encoding
	$\enco{\map{\cdot}^2, \mapa{\cdot}^2, \mapa{\cdot}^2}: \HO \to \sessp$
	with mappings 
...
%			   $\stytra{\mapt{\Gamma}^{3}}{\mapa{\ell_1}^{3}}{\mapt{\Delta}^{3}}{\map{P}^{3}}{\mapt{\Delta'}^{3}}{$.
		...
		\item	 
\dk{			If  $\stytra{\Gamma}{\ell_1}{\Delta}{P}{\Delta'}{P_1 \Par P_2\subst{\abs{x}Q}{X}}$
			with $\ell_1 = \tau$
			then \\
			$\mapt{\Gamma}^{2};\, \mapt{\Delta}^{2} \proves  \map{P}^{2}
			\hby{\mapa{\ell_1}^{2}}
