...
preserve not only syntactic criteria, but
also the operational semantics and typing.
We first state syntactic criteria \\ \jp{Still need to define what renaming policies makes sense for our typed setting!}
...
\begin{definition}[Syntax Preserving Encoding]\rm
	...
	\begin{enumerate}[1.]
		...
		\item	\emph{Compositionality wrt Restriction}: If 
		...
		\item A renaming policy - TBD.
		Let $\sigma$ denote a substitution on names.
		If 
		$\mapt{\Gamma}; \emptyset; \mapt{\Delta} \proves \map{\sigma(P)} \hastype \Proc$
		then \\
		$\mapt{\Gamma}; \emptyset; \mapt{\Delta} \proves \sigma(\map{P}) \hastype \Proc$.
...
\begin{proposition}[Operational Correspondence, \HOp into \HO]\rm
	...
	\begin{enumerate}[1.]
		\item
			...
			\Par R
			}$, where $R \Hby{} \pmapp{P_2}{1}{f}\subst{m}{x}$.
