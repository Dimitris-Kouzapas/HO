...
called context bisimulation~\cite{San96H}:
%
...
Consider, e.g., $S = \btinp{\shot{S_1}} \btout{S_2} \tinact$, the 
session type that
firsts input an abstraction, from type $S_1$ to a process, then 
...
characteristic processes; if under the substitution
...
\label{def:dettrans}
	...
	\begin{enumerate}[$-$]
		\item	{\em Session transition}, written
			...
			whenever the untyped transition $P \by{\tau} P'$ using 
			rule~$\ltsrule{Tau}$ 
		(where $\subj{\ell_1}$ and $\subj{\ell_2}$ in the premise 
		are dual endpoints), 
		possibly followed by uses of  $\ltsrule{Alpha}$, $\ltsrule{Res}$, $\ltsrule{Rec}$, or $\ltsrule{Par${}_L$}/
