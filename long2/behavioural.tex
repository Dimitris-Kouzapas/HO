...
and ii) the set of higher order input actions
on an input prefix is infite since infinetely
many processes can match the session type of
the input prefix.

To define a easy to compute bisimulation we need to
avoid the universal quantifier on theb output case and
restrict the set of input processes on an input
prefix.
...
For example the simplest process that satisfies the type
$\btinp{\shot{S_1}} \btout{S_2} \tinact$ is
$\binp{u}{x} (\bout{u}{s_2} \tinact \Par \appl{x}{s_1})$.
...
The aim is to use the characteristic process as a basis for
restricting the input action on processes so we can define
a bisimulation relation that does not suffer from infitely
many input abstractions.
...
To solve this problem we introduce the trigger input.
\dk{Put intuition about trigger input}
