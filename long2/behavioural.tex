...
\begin{example}
	...
	i.e.~input $\abs{x} \bout{a}{s_3} \btinp{x}{y} \inact$.
\end{example}
...
\begin{eqnarray*}
	...
	\\
	\Gamma; \es; \Delta \proves \newsp{s}{\binp{n}{x} \appl{x}{s} \Par \bout{\dual{s}}{\abs{x} Q} \inact}
\end{eqnarray*}
%
\noi can both observe the trigger input and get
\begin{eqnarray*}
...
but for input $\abs{z}{\binp{z}{x} \appl{x}{m}}$
...
%and shown to be bisimilar regardless of the type of $n$.
...
As we discussed previously it is convienient to define
...
\begin{definition}{Bisimulation}\rm
	\label{def:bisim}
	Typed relation
	$\Re$ is a {\em bisimulation} if for a trigger $\trigger{t}{V}{U}$, whenever
