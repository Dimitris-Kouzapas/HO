...
we consider a usecase scenario that adapts the example developed by Mostrous and Yoshida~\cite{tlca07,MostrousY15}.
...
Process $\Client_1$ sends two abstractions with body $P_{xy}$, one to each hotel, 
	...
	Process $P_{xy}$ is the mobile code: % responsible for negotiating with a hotel
	while
...
The differences between $\Client_1$ and $\Client_2$ can be easily seen in the sequence diagrams of \figref{fig:exam}. 
