...
%				and,
	...
	\begin{frame}{Higher-Order Contextual Bisimulation: Input}
		...
		\]
		\begin{itemize}
			\item	Choose the {\em simplest} process $V$ that inhabits $U$ so to improve
				the \textcolor{blue}{$(\bullet)$} clause
		\end{itemize}
	\end{frame}
			...
			\[
				\tree {
					\begin{array}{c}
						...
						\vee& V &\scong& \abs{x}{\binp{t}{y} (\appl{y}{x})} \textrm{ with $t$ fresh}
						\end{array}
					\end{array}
				}{
		...
		\begin{itemize}
			...
			\item	If an instance of the trigger value inputs a trigger value
				\begin{eqnarray*}
					...
					\Delta \cat t_2: \btinp{U} \tinact \proves \binp{t_2}{y} (\appl{y}{n}) %\appl{\abs{x}{\binp{t'}{y} (\appl{y}{x})}}{n} 
				\end{eqnarray*}
%			\item	Lack of the trigger value in the above definition would result
%				in an under-discriminating equivalence relation (example~12 in the paper)
		\end{itemize}
