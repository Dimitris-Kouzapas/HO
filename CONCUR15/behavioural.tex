...
%\myparagraph{Labels.}
%\noi 
We define an (early) labelled transition system (LTS) over
untyped processes. 
Later on, using the \emph{environmental} transition semantics, 
we define a typed transition relation to formalise 
how a typed process interacts with a typed observer. 
%process in its environment. 
Interaction is defined on actions $\ell$:
\begin{center}
\begin{tabular}{l}
	$\ell	\bnfis   \tau 
		\bnfbar	\bactinp{n}{V} 
...
Characteristic  Bisimulation ($\fwb$)}\label{ss:hwb}
\noi 
...
\noi Note 
$\horel{\Gamma}{\Delta_1}{P_1}{\hby{\ell}}{\Delta_2}{P_2}$ (refined input) implies  
$\horel{\Gamma}{\Delta_1}{P_1}{\by{\ell}}{\Delta_2}{P_2}$ (ordinary input).
...
We use an auxiliary equivalence called \emph{higher-order bisimilarity} (denoted~$\hwb$, see \defref{def:bisim}) which 
