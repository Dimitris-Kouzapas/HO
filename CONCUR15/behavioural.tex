...
reduces it following \defref{d:wtenvred}.
...
\noi 
We formalise the ideas motivated in % \secref{sec:overview}.
the introduction.
...
We show internal transitions associated to session interactions or  
...
We use an auxiliary equivalence called \emph{higher-order bisimilarity} (denoted $\hwb$, see \defref{def:bisim} in the Appendix) which 
...
\begin{proof}[Proof (Sketch)]
	...
	More details  can be found in \appref{hotel_closure}.
...
&		& \Par \ftrigger{t_1}{\abs{x}{P \subst{h_1}{y}}}{\lhot{S}} \Par \ftrigger{t_2}{\abs{x}{P \subst{h_2}{y}}}{\lhot{S}}}
