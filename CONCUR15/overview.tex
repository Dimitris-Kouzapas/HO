...
The second issue follows the fact that we work with an \emph{early}
labelled transition system (LTS), thus an higher-order input prefix observes
finitely many different input values.
To reduce the above burdens,
%induced by 
...
%($s$ and $\dual{s}$) 
are deterministic.  
Below we write $Q \by{\bactinp{n}{V}} Q'$ to denote an input transition along $n$.
Now let us
