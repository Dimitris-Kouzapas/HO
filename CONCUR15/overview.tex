...
%\noi 
\smallskip 
In the remainder of the introduction, we explain how our approach exploits session types to 
...
There are two issues, associated to demanding requirements appearing in clauses for output and input actions. 
...
Suppose $P \,\Re\, Q$, for some context bisimulation $\Re$. Then we have:
...
\noi 
where
$\news{\widetilde{m}} \bactout{n}{V}$ is the output label of 
value $V$ 
along $n$
with extrusion of names in $\widetilde{m}$. 
The \emph{second issue} 
appears in input clauses, and 
follows from the fact that we work with an \emph{early}
labelled transition system (LTS). Thus, a higher-order input prefix may observe
finitely many different input values.
...
\begin{enumerate}[(a)]
	...
	\item We refine  input transitions 
...
Below, we 
write $\binp{s}{x}{P}$ 
for an input on (linear) endpoint $s$, 
and $\bout{\dual{s}}{V} Q$
for an output of value $V$ 
on endpoint $\dual{s}$ (the dual of~$s$).
Also, we write $R \by{\bactinp{n}{V}} R'$ to denote an input transition along $n$.
...
output clause $(\star)$. 
%However, 
Trigger process $T_{V}$ in~\eqref{equ:2} requires a higher-order communication along $t$.
...
\myparagraph{Characteristic Trigger Process}
