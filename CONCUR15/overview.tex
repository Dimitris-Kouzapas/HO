...
%($s$ and $\dual{s}$) 
are deterministic.  
Below we write $P \by{\bactinp{n}{V}} P'$ to denote an input transition along $n$.
Now let us
...
%\hotrigger{t}{V_1} 
T_{V} = \hotrigger{t}{V} \quad\mbox{and}\quad 
T_{V}
...
output clause $(\star)$. 
However, the trigger $T_{V}$ in \eqref{equ:2} requires a higher-order communication along $t$.
