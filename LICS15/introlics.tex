...
in particular via 
comparisons with the first-order mobility of the $\pi$-calculus~\cite{MilnerR:calmp1}. 
...
Thus, the calculus $\sessp$ is comparable to the calculus in~\cite{honda.vasconcelos.kubo:language-primitives}, 
the \HO calculus is a 
...
The theory that we develop exploits the protocol specifications given by session types to  limit 
the behavior of higher-order session processes. 
The limitation induced by session types allows to narrow down the spectrum of allowed process behaviors, 
thus naturally enabling tractable reasoning techniques. 
Two characterizations of bisimilarity, 
targeted to higher- and first-order session processes,
shown to coincide with contextual equivalence.
Then, in the second part of the paper we assess the expressive 
power of the higher-order communication as delineated by typing. 
Encodability results explain the relationships between \HOp, \HO, and \sessp
via fully abstract encodings. 
A non encodability result formalizes the expressive power added by shared names.
In the remainder of the introduction, we motivate further these contributions. 
