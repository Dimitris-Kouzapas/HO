...
\dk{Encoding of a recursive process $\recp{X}{P}$ is another challenge, since 
it must preserve the linearity of session endpoints appeared in $P$.
We encode it with non-tail recursive session types,  
for which we apply a recent advance on a session duality theory \cite{DBLP:journals/corr/abs-1202-2086,TGC14}.}
