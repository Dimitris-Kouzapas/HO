...
between (higher-order) session calculi. Our discovery presented 
in this paper is that linearity of session types plays a vital role to 
offer new, fully abstract encodability results, not proposed before.   
...
recursion, 
name abstractions (i.e., functions from name identifiers $x$ to processes, $\lambda x.P$) and applications $(\lambda x.P)a$, in the call-by-value style;  
...
the \HO-calculus, which is \HOp without recursion and name passing,
the \sessp-calculus, which is \HOp without abstractions and applications.  
...
$\pi$-calculus can be encodable into the $\pi$-calculus, our main result 
is encoding \HOp into \HO where name-passing is absent, 
...
and $\newsp{s}P$ is hiding. 
Here we need exactly two (deterministic) reductions between session endpoints 
($s$ and $\dual{s}$) to unpack name $a$.
