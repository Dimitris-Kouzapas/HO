...
The work~\cite{XuActa2012} studies the encodability of the higher-order $\pi$-calculus (with a relabelling operator) into the first-order $\pi$-calculus; encodings in the reverse direction are also proposed. %, following \cite{Tho90}.
A core calculus of higher-order concurrency is studied in~\cite{DBLP:journals/iandc/LanesePSS11}: it lacks restriction,  name passing, output prefix (communication is asynchronous), and constructs for infinite behaviour. 
This calculus (a sublanguage of \HO) has 
a simple notion of (strong) bisimilarity which coincides with reduction-closed, barbed congruence.
%be Turing complete, while 
%have a decidable notion of (strong) bisimilarity that coincides with barbed congruence. 
\jpc{The results of~\cite{DBLP:journals/iandc/LanesePSS11} do not consider types.} 
