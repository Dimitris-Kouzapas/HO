...
(\defref{d:enc:hopitopi}) and behavioral equivalences
...
The work~\cite{XuActa2012} studies the encodability of the higher-order $\pi$-calculus (with a relabeling operator) into the first-order $\pi$-calculus; encodings in the reverse direction are also proposed. %, following \cite{Tho90}.
...
while our language lacks matching. To compensate for
the absence of matching, we use the characteristic
process interaction with the environment, i.e., i.e instead of
name matching a name is embedded into a process and then observe its behavior.
