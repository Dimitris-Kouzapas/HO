...
a higher-order $\pi$-calculus adding a powerful 
name relabelling operator; 
these relabellings are shown to be essential to encode name-passing, 
for they add expressiveness to 
higher-order processes}. %, following \cite{Tho90}.
...
\NY{The paper~\cite{DBLP:conf/wsfm/XuYL13} 
%complements~\cite{DBLP:conf/icalp/LanesePSS10} 
studies the expressive power of second-order abstractions with
replication ($!P$).  
...
It is shown that polyadicity of process abstraction induces an
expressiveness hierarchy, and that name abstraction with
replications can encode process abstraction. In contrast, we encode 
\PHOpp fully abstractly into \HO where either recursion or
replication is absent.}
The works~\cite{DBLP:conf/icalp/LanesePSS10,DBLP:journals/iandc/LanesePSS11,DBLP:conf/icalp/LanesePSS10,DBLP:conf/wsfm/XuYL13} focus on untyped processes;
\NY{none of the above works proposed tractable bisimulations like
$\hwb$ and $\fwb$, which only require observation of a finite number of 
higher-order values,  
whose formulations rely on session type strucures.}
...
The discipline developed for $\HOp$ is a subset of that in~\cite{tlca07,MostrousY15}.
...
also reasoned in 
$\sessp$ and $\HO$ (with limited form of arrow types, $\shot{C}$ and $\lhot{C}$), via precise encodings. \dk{None of the above works proposes tractable 
bisimulations for higher-order session processes.}  
