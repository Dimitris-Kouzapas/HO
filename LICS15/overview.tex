...
This way, e.g., if session type $S = \btinp{\shot{S_1}} \btout{S_2} \tinact$
(which means that inputing a function, %from values $S_1$ to processes, 
...
\dk{An important point to stress-out is that our calculus lacks
a matching construct
which is traditionally a crucial element to prove completeness of bisimilarity.
Nevertheless, we use types and specifically the characteristic process
to compensate for the absence of matching, i.e., i.e instead of
name matching we use a process trigger to embed a name into a characteristic
process and then observe its behavior
