...
processes to processes with no free names (but with free variables) (\defref{d:auxmap}). 
...
\caption{\label{f:auxmap} An auxiliary mapping used in \defref{d:enc:hopitoho}.}
\Hline 
\end{figure}
...
\dk{Non-tail
recursive session type were studied in
\cite{DBLP:journals/corr/abs-1202-2086,DBLP:journals/corr/BernardiH13},
but this is the first application in the
...
\dk{i) record a mapping from recursive variable $X$ to process variable $z_X$;
ii) encode the recursion body $P$ as a (polyadic) name abstraction
in which free names of $P$ are converted into name variables;
iii) this higher-order value is embedded in a sort of input-guarded 
``duplicator'' proces;
iv) the encoding of process variable $z_X$ is then meant to 
...
the application of $\fn{P}$ and the other copy used to re-invoke
