...
\begin{definition}[Typed Calculus]\label{d:tcalculus}\rm
A \emph{typed calculus} $\tyl{L}$ is a tuple
          $\calc{\CAL}{\cal{T}}{\hby{\mathcal{A}}}{\wb}{\proves}$
	where $\CAL$ and $\cal{T}$ are sets of processes and types, 
respectively; and $\hby{\mathcal{A}}$, $\wb$, and $\proves$ 
	denote a transition system (with set of labels $\mathcal{A}$), a typed equivalence, and type system for $\CAL$, resp. 
\end{definition}
...
respectively; and $\hby{\mathcal{A}}$, $\wb$, and $\proves$ 
	...
	We write $\mathcal{A}$ is the set of labels used in relation $\hby{\ell}$.
\end{definition}
