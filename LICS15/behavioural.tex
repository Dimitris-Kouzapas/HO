...
\myparagraph{Output Actions} are defined by $\eltsrule{SOut}$
and $\eltsrule{ShOut}$.  
...
We first give an example that extends the motivation given in \S\,\ref{sec:intro}; then we define characteristic processes.
\smallskip  
...
\end{eqnarray}
%
We can show that $\Gamma; \es; \Delta \cat s: \btinp{\shot{\btinp{C} \tinact}} \tinact \proves P_i \hastype \Proc$.
...
are clearly {\em not} contextually bisimular: there exist many input actions
which may be used to distinguish them.
For example, if 
$P_1$ and $P_2$ input 
 $\abs{x} \bout{a}{s_3} \binp{x}{y} \inact$ then
their derivatives not bisimilar. 
...
processes (\ref{equ:5}) and (\ref{equ:6}):  
%
\begin{eqnarray*}
...
\end{eqnarray}
%
\noi if both processes in (\ref{equ:7}) and (\ref{equ:8}) 
...
%	with 
	...
	We define the {\em characteristic process} 
	$\mapchar{U}{u}$ with %{\em characteristic value}
	$\omapchar{U}$ in Fig.~\ref{fig:char}. 
\end{definition}
...
\begin{enumerate}
$\Gamma; \emptyset; \Delta \cat u:S \proves \mapchar{S}{u} \hastype \Proc$; and $\Gamma \cat u:\chtype{S}; \emptyset; \Delta \proves \mapchar{\chtype{S}}{u} \hastype \Proc$; and 
...
$\eltsrule{RRcv}$. %to $\eltsrule{SRcv}$ and $\eltsrule{ShRcv}$. 
We then define 
$\horel{\Gamma}{\Delta_1}{P_1}{\hby{\ell}}{\Delta_2}{P_2}$
