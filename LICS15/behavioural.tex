...
processes called {\bf\em characteristic values} which are explained 
...
\begin{definition}[Characteristic Process and Value]\rm
...
a mapping $\omapchar{U}$ for a {\em characteristic value} 
in Fig.~\ref{fig:char}. 
\end{definition}
...
(a \textrm{ fresh})\quad\quad
	...
	\end{array}
	\]
\caption{Characteristic Processes (top) and Values (bottom) \label{fig:char}}
\Hline
\end{figure}
...
the characteristic value, $\lambda x.\mapchar{U}{x}$. 
Hence with the same reasoning as (\ref{eq:1})--(\ref{eq:3}), 
we define the {\bf characteristic trigger process} with type 
...
the processes in (\ref{eq:5}) and (\ref{eq:6}) 
that is if a trigger value is received, 
we obtain:
%
\begin{eqnarray*}
...
$\abs{{x}}{\mapchar{U}{x}}$ as an input observation in \eltsrule{RRcv}.  
\end{example}
