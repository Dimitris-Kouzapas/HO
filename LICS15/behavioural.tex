...
\label{ss:elts}
\noi 
We define a labelled transition relations between 
...
and ii) a higher order input prefix which can observe
...
\subsection{The HO Bisimulation: $\hwb$}\label{ss:hwb}
\noi 
To overcome the problem of $\wbc$, we introduce two 
higher-order bisimulations. The first one is 
is called the {\em higher-order bisimulation}. 
We observe that 
...
To formalise this idea, we introduce the following restricted 
