...
Clause 1) in Definition~\ref{def:wbc} is equivalent to 
\begin{equation}\label{eq:1}
\newsp{s}{\appl{(\abs{x}{R})}{s} \Par \bout{\dual{s}}{V_1} \inact}
\end{equation}
since $s$ is a session name. Hence using the fresh trigger name $t$, 
if $\hotrigger{t}{V_1}$ receives
$\abs{x}{\binp{t}{X} \appl{X}{x}}$ 
