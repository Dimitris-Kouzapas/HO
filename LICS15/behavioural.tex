...
barbed congruence (Theorem~\ref{the:coincidence}).
...
We write $\fn{\ell}$ and $\bn{\ell}$ to denote the
 sets of free and bound names in $\ell$, resp.
...
we denote $\Delta_1 \bistyp \Delta_2$ if there exists $\Delta$ such that
	...
	\jpc{(here we write $\red^\ast$ for the multistep reduction in \defref{def:ses_red})}.
%\end{definition}
...
\dk{for proving our expressiveness results (\S\,\ref{sec:positive})}.
