...
Clause 1) in Definition~\ref{def:wbc} is bisimular to the following process, with fresh $s$,  
...
\newsp{\tilde{m_1}}{P_2 \Par R\subst{V_1}{x}}
\end{eqnarray*}
The process in (\ref{eq:1}) is regarded as a {\em trigger process} which 
...
is not sufficient to reason about \sessp 
...
For the third bisimulation, called the {\em first-order bisimulation}, 
...
rules using the environment transition relation defined in Definition~\ref{}:
\[
\begin{array}{rl}
		\eltsrule{RRcv}&\tree {
\begin{array}{c}
...
The third bisimulation is given using the characteristic processes. 

\begin{definition}[First-Order Bisimulation]\rm
\label{d:fwb}
The first order bisimilarity, denoted by $\fwb$, is defined replacing 
Clause 1) in Definition~\ref{d:hwb} by the following clause:
	\begin{enumerate}
		\item	whenever 
