...
\subsection{Two Labelled Transition Systems for Environments}
\label{ss:elts}
\noi 
We define the two labelled transition relations between 
a triple of environemnts. The first one is standard 
linear LTS; and the second one is more {\em efficient} since 
it limits the input labels based on the notion of triggers and 
characteristic processes. 

\myparagraph{Transition System (1)}
We define the first transition system 
...
the resulting session environment. 
...
\myparagraph{Efficient Transition System}
