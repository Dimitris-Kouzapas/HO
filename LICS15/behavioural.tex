...
\subsection{HO Bisimulation ($\hwb$) and FO Bisimulation ($\fwb$)}\label{ss:hwb}
\noi 
To overcome the problem of $\wbc$, we introduce two bisimulations.
...
For 1), we first 
observe that 
the output action case of $\wbc$ 
can be equivalently be defined as follows:\\
...
\]
%
This is because we can observe an input on a fresh name $t$
of any process $R$ such that $\set{x} = \fv{R}$, and the 
reduction between $s$ and $\dual{s}$ is deterministic. 

On the other hand, only observing a trigger value and channel 
is not enough is not sufficient since we equate too many processes
(see Example  



The reason will be explain in 
Example~\ref{ex:motivation}. 
...
uses even simpler values which are defined 
using session types. This simple process, called a {\em characteristic process}, 
