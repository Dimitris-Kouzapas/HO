...
object. Thus, output action is dual with input action and 
select action is dual with branch action.
Formally, the dual action relation is the symmetric relation $\asymp$ that satisfies the rules:
\[
...
observable action. 
If a restricted name occurs free in an output
action then the name is added in the bound name list of the action
and the continuation process performs scope opening (rule $\ltsrule{New}$).  
...
internal transition.  Rules $\ltsrule{Par${}_L$}$ and $\ltsrule{Par${}_R$}$ 
close the LTS under the parallel operator provided that the observable
...
environments (Definition~\ref{def:ses_red}), as seen in rule
$\eltsrule{Tau}$.
...
\begin{enumerate}
	\item	replace 
		...
		by a more tractable closure; and 

	\item	limit the input value $V$ in $n?\ENCan{V}$ in 
		rules $\eltsrule{SRv}$ and $\eltsrule{ShRv}$ in Fig.~\ref{fig:envLTS}.
\end{enumerate}
%
For 1), we first 
...
offers value $V_1$ at fresh name $t$ \cite{JeffreyR05}, so that we write as: 
...
{\bf\em the characteristic value}, $\omapchar{U}$.%$\lambda x.\mapchar{U}{x}$. 
If we follow the same reasoning as (\ref{eq:1})--(\ref{eq:3}), 
...
\begin{eqnarray}
	...
	\label{eq:5}
	\\
	&&\hspace{-1cm}\Gamma; \es; \Delta \cat s: \btinp{\shot{\btinp{C} \tinact}} \tinact \proves \binp{s}{x} (\appl{x}{s_1} \Par \binp{s_2}{y} \inact) \hastype \Proc
	\label{eq:6}
\end{eqnarray}
%
If the above processes input and substitute over $x$
the characteristic value $\abs{z}{\binp{z}{y}} \inact$, 
then both the processes continue as:%(\ref{eq:5}) and (\ref{eq:6}) in become:
%
\begin{eqnarray*}
...
both process become contextually bisimilar. 
The processes in (\ref{eq:5}) and (\ref{eq:6}) 
are obviously {\em not} contextually bisimular since
for other numerous other input processes, they can be distinguished. 
...
processes (\ref{eq:5}) and (\ref{eq:6}):  
%
\begin{eqnarray*}
...
\noi However if both processes input $\abs{z}{\binp{z}{x} (\appl{x}{m})}$,  
then the resulting processes become:
\begin{eqnarray*}
