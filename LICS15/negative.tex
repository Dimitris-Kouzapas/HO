...
This result begs the question: \dk{can we further encode the replicated
triggers in the encoding from \HOp to \HO using only session names?}
...
The proof of Theorem~\ref{t:negative} relies on
%As described next, 
...
Then there exists a precise encoding 
from 
$\tyl{L}_{\CAL_1^{-\mathsf{sh}}}$ 
into $\tyl{L}_{\CAL_2^{-\mathsf{sh}}}$.
\end{corollary}
