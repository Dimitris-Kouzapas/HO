...
%			\map{\appl{X}{\tilde{k}}}^{p}  &\defeq& \newsp{s}{\appl{X}{s} \Par \map{\bout{\dual{s}}{\tilde{k}} \inact}^{p}} \\
	...
	\end{array}
	\]
	and as an homomorphism for the remaining constructs in \pHOp. \jp{I prefer to be explicit in the encoding of polyadic abstraction/applications. Previous version is commented.}
\end{definition}
...
\jp{Not sure the ordering mapping is properly defined - I have messed up some command name. Below, what are $L_{\HOp}$, $L_{\HOp}$??}
...
operators:
\jp{Notice that this needs to be parametric on a set of names, $N$.}
\begin{definition}[From $\sesp$ to $\pHOp$]
