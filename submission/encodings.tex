...
%	\map{\rvar{X}}^{2} &=& \newsp{s}{\appl{\X}{s \cat \smap{\fn{P}}} \Par \bout{\dual{s}}{ \abs{z \cat \vmap{\fn{P}}}{\appl{X}{z \cat \vmap{\fn{P}}}}} \inact} \\
	\pmapp{\rec{X}{P}}{2}{f} &=& \newsp{s}{\binp{s}{\X} \pmapp{P}{2}{{f,\{\rvar{X}\to \vmap{\ofn{P}}\}}} \Par \bbout{\dual{s}}{\abs{z , \vmap{\ofn{P}}}{\binp{z}{\X} \auxmapp{P}{\sigma}{\es}}} \inact} \vspace{1mm}\\ 
...
\begin{proposition}
	Encoding $\fencod{\cdot}{\cdot}{2}{f}: \sesp \to \HO$  is type-preserving (cf. Def.~\ref{def:ep}\,(1)).
\end{proposition}
...
%	\dk{TODO}
%\end{proof}
(Mapping $\pmap{\cdot}{n}$ would be defined homomorphically for the remaining $\HO$ constructs.)
Although this mapping captures the correct semantics when
...
%%%% Output of (linear) channel
	...
	\end{enumerate}
	\jp{Check how the encoding affects $\Gamma$ and $\Lambda$!}
	\qed
	\end{proof}
