...
The session type system for \pHOp is an orthogonal extension of that in \S\,\ref{s:types}.
The type syntax for values is extended as follows, where $\tilde{S}$ stands for a sequence $S_1, \ldots, S_n$ of 
session types:
\begin{eqnarray*}
...
\begin{definition}[Polyadic Into Monadic]Define the process mapping $\auxmap{\cdot}{\mathsf{p}}:\pHOp \to \HOp$ as
\[
	\begin{array}{rcl}
...
%			\map{\appl{X}{\tilde{k}}}^{p}  &\defeq& \newsp{s}{\appl{X}{s} \Par \map{\bout{\dual{s}}{\tilde{k}} \inact}^{p}} \\
	...
	Define the mapping on types $\tauxmap{\cdot}{\mathsf{p}}$ as follows:
\[
	\begin{array}{rcl}
			\tauxmap{\btout{S_1, \cdots, S_n}S}{\mathsf{p}}&\defeq& \btout{\tauxmap{S_1}{\mathsf{p}}} \cdots \btout{\tauxmap{S_n}{\mathsf{p}}}\tauxmap{S}{\mathsf{p}} \\
			\tauxmap{\btinp{S_1, \cdots, S_n}S}{\mathsf{p}}&\defeq& \btinp{\tauxmap{S_1}{\mathsf{p}}} \cdots \btinp{\tauxmap{S_n}{\mathsf{p}}}\tauxmap{S}{\mathsf{p}} \\
			\tauxmap{\lhot{(S_1, \cdots, S_n)}}{\mathsf{p}}&\defeq& \lhot{\big(\btinp{\tauxmap{S_1}{\mathsf{p}}} \cdots \btinp{\tauxmap{S_n}{\mathsf{p}}}\tinact\big)} \\
			\tauxmap{\shot{(S_1, \cdots, S_n)}}{\mathsf{p}}&\defeq& \shot{\big(\btinp{\tauxmap{S_1}{\mathsf{p}}} \cdots \btinp{\tauxmap{S_n}{\mathsf{p}}}\tinact\big)} 
	\end{array}
	\]
...
For the sake of completeness, we give again the encodings for finite processes and types, as
...
\begin{remark}
It seems that we would need to define a mapping for environments $\Gamma$, as follows:
$$
\tmap{\Gamma \cat \rvar{X}:\Sigma}{2} = \tmap{\Gamma}{2} \cat 
X:\shot{(\tilde{S}_{\Sigma}, S^*)}
