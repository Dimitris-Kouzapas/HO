...
\begin{definition}\label{d:trabs}
   Let $\sigma$ be a set of session names.
	...
	\begin{array}{rcll}
		...
		\left\{
		\begin{array}{rl}
		    \bout{s}{s'}P & s, s' \in \sigma \\
		    \bout{x_s}{s'}P & s \not\in \sigma, s' \in \sigma \\
		    \bout{s}{x'_s}P & s \in \sigma, s' \not\in \sigma \\
		    \bout{x_s}{x'_s}P & s, s' \not\in \sigma 
		\end{array}
		\right.
		\\
...
$N_P = \vmap{\ofn{P}}$.
Also, let $A_P$ be the polyadic abstraction $\abs{N_P}\auxmapp{P}{\emptyset}{\mathsf{v}}$ (cf. Def.~\ref{d:trabs}).
