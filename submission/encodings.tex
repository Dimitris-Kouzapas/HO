...
\subsection{Polyadic Variances}

Polyadicity is one of the first extensions that were proposed
for the $\pi$-calculus, \dk{\cite{}}. We call polyadic semantics,
the semantics of a $\pi$-calculus that allow us to pass as a
message, a list of names rather than a single name.
Polyadicity is shown to be
encodable in the monadic case and it is broadly studied
and established as a sound semantic approach.
In the context of session types polyadic communication
has an {\em easier} encoding than in $\pi$-calculus,
...
\begin{definition}[Polyadic mapping]
	We write $\map{\cdot}^{p}:\pHOp \to \HOp$ whenever
...
%		\begin{tabular}{l}
			...
			$\map{\bout{k}{\abs{\tilde{x}} P_1} P_2}^{p}$ &$\defeq$& $\bout{k}{\abs{z}\map{\binp{z}{\tilde{x}} P_1}^{p}} \map{P_2}^{p}$\\
...
\begin{definition}
	...
	\begin{array}{rcll}
		...
		\left\{
		\begin{array}{rl}
			\bout{x_s}{\abs{\vmap{x, \fn{P}}} \map{P'}^{\es}} \map{P}^{\sigma} & s \notin \sigma\\
			\bout{s}{\abs{\vmap{x, \fn{P}}} \map{P'}^{\es}} \map{P}^{\sigma} & s \in \sigma
		\end{array}
		\right.
		\\
		...
		\map{P_1 \Par P_2}^{\sigma} &\bnfis& \map{P_1}^{\sigma} \Par \map{P_2}^{\sigma} \\%& s \notin \sigma\\
	...
	\end{array}
\]
where map $\smap{\cdot}: 2^{\mathcal{N}} \longrightarrow \mathcal{N}^\omega$
orders lexicographically a set of names and map 
...
\begin{proof}
	...
	\map{P}^{\sigma} \subst{\smap{\fn{P}}}{\vmap{\fn{P}}}$ \dk{TODO}
\end{proof}
