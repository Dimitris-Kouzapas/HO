...
types. 
The interaction of processes with their environment is defined using action labels $\ell$:
%
\begin{center}
	\begin{tabular}{l}
		$\ell
...
\secref{sec:types}.
The LTSs follow the intuitions given in \secref{ss:equiv}, with underlying sets of actions $\mathcal{A}_{\HOp}$, $\mathcal{A}_\HO$, and $\mathcal{A}_\sessp$ defined as expected.
...
these two encodings may be combined to relate the super-calculus \PHOpp with the encodings into \HO and \sessp, given in the previous section.
...
$\mathcal{A}_\HOpp$
includes output and input actions
$\news{\tilde{m}} \bactout{n}{\abs{\AT{x}{L}}{P}}$
and
${\bactinp{n}{\abs{\AT{x}{L}}{P}}}$.
...
\thmref{f:enc:hotopi}, 
and 
\thmref{f:enc:hoppptohop} (which give preciseness for all the encodings), using
...
extends $\HOp$ 
with polyadicity.
The syntax of $\PHOp$ thus enables the exchange of tuples of names $\widetilde{n}$ (with fixed length $k \geq 1$) in session communication (but not along shared names), 
and extends the  syntax 
of values $V$
with polyadic first-order applications of the form
 $\abs{x_1, \ldots, x_k}{Q}$, with $k \geq 1$.
The operational semantics is kept unchanged, with the expected use of the simultaneous substitution $\subst{\widetilde{V}}{\widetilde{x}}$.
...
%\figref{fig:typerulesmy} 
in \secref{sec:types}
with the polyadic types given above.
Also, the set of labels 
$\mathcal{A}_{\PHOp}$
includes actions
...
is defined as follows:
	...
	\end{align*}
	and as an homomorphism for all other cases of $\ell \in \mathcal{A}_{\PHOp}$.
	TO BE COMPLETED.
\end{definition}
