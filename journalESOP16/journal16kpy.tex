...
actions from a finite set $\cal{A}$, which contains at least $\tau$. We will often be interested in those $\tau$-transitions, which characterise reductions. 
We shall write $\hby{\tau}$ to make this choice explicit. Nevertheless, to state more precise forms of operational correspondence, we 
will sometimes find it convenient to use transitions of 
the form $\hby{\alpha}$,
where $\alpha \in \cal{A}$ and $\alpha \neq \tau$ (i.e., visible transitions). 
...
our notion of typed encoding shall include mappings $\map{\cdot}$ and $\mapt{\cdot}$, but also a
a  mapping  $\mapa{\cdot}:\mathcal{A}_1 \to \mathcal{A}_2$ connecting the sets of actions in source and target typed calculi.
...
We now state a generalized form of operational correspondence, which includes reductions ($\tau$-labeled transitions) but also other transitions.
%Before we prove operational correspondence we
We define mapping from $\mapa{\cdot}^{1}: \mathcal{A} \to \mathcal{A}$
...
\begin{proof}[Sketch]
Recall that forms of labeled operational correspondence for each encoding have been
stated in 
\propref{prop:op_corr_HOp_to_HO}
and 
\propref{prop:op_corr_HOp_to_p}.
Now, to substantiate the above claim, we show that the encoding $\map{\cdot}^1_f$ enjoys 
