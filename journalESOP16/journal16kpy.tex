...
We state key definitions and properties for session type system for \HOp.
Our system,
introduced in~\cite{KouzapasPY17},
 distills the key features of~\cite{tlca07,MostrousY15} and so it is simpler.
See \appref{app:types} for definitions of type equivalence and type duality, and 
...
We define \emph{characteristic} and \emph{higher-order} bisimilarities,
%While higher-order bisimilarity is a new equality, 
as jointly introduced in~\cite{KouzapasPY17}. % (Def.~14).
...
two dual endpoints), while 
 ...
 We write  $\horel{\Gamma}{\Delta}{P}{\hby{\dtau}}{\Delta'}{P'}$ to denote
	...
	See~\cite{KouzapasPY15} for definitions of $\hby{\btau}$  and $\hby{\stau}$. % rely on an LTS for \HOp;  for details.
 \end{notation}
...
\begin{proof}
\propref{app:prop:typepres_HOp_to_HO}.
\end{proof}
...
\begin{proof}
See \appref{app:enc:HOp_to_sessp}.
\end{proof}
