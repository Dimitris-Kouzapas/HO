...
actions from a finite set $\cal{A}$, which contains at least the unobservable action $\tau$. 
...
describing how the actions in the source calculus are mapped in the target typed calculus.
...
In our concrete case,  $\sigma(\Gamma)$ clearly concerns only shared names in $\Gamma$: process and recursive variables in $\Gamma$ are not affected by $\sigma$. 
...
Type preservation is key to retain the meaning of structured protocols:
