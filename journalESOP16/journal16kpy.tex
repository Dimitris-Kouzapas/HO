...
two dual endpoints), while 
 ...
 We write  $\horel{\Gamma}{\Delta}{P}{\hby{\dtau}}{\Delta'}{P'}$ to denote
	...
	See~\appref{app:behavioural} and \cite{KouzapasPY17} for formal definitions of $\hby{\btau}$  and $\hby{\stau}$. % rely on an LTS for \HOp;  for details.
 \end{notation}
...
First, by considering the encoding of $P_1$ into \HO  (following mapping $\pmap{\cdot}{1}_f$ in \figref{f:enc:hopi_to_ho})
we obtain:
\begin{eqnarray*}
...
we may 
distinguish the precise encodings $\map{\cdot}_f^1$ and $\pmap{\cdot}{2}$ depending on whether they are also tight encodings or not (cf. \defref{def:lopco}):
...
We are grateful to the %ESOP'16 
