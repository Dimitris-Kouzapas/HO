...
\author[gl]{Dimitrios Kouzapas\corref{cadk}}
\cortext[cadk]{Corresponding author}
\address[gl]{Department of Computer Science, University of Cyprus, Nicosia, Cyprus
}

\author[gr]{Jorge A. P\'{e}rez\corref{cajp}}
\cortext[cajp]{Corresponding author}
\ead{j.a.perez@rug.nl}
\address[gr]{University of Groningen \& CWI, Amsterdam, The Netherlands\\
}

\author[ic]{Nobuko Yoshida\corref{cany}}
\cortext[cany]{Corresponding author}
...
implement complex programming abstractions using simpler process constructs.
...
session types for the %(first-order) 
$\pi$-calculus describe the intended interactive behaviour of the names %/channels 
...
up to two
behavioural equivalences that characterise barbed congruence:
...
\begin{enumerate}[(i)]
\item
the encodability of \HO into \sessp (\secref{subsec:HOp_to_sessp}); 
\item
a non encodability result showing that shared names strictly add expressive power to session calculi (\secref{ss:negative}).
\item
extensions of our encodability results to richer settings (\secref{sec:extension});
\end{enumerate}
...
\newj{\figref{fig:express} summarises our encodability results.}
...
the non encodability result (ii),
...
{Omitted definitions and  proofs are in the Appendices (\ref{app:behavioural} and \ref{app:proofs}).
...
Below, 
$a,b$ are names and $s$ is  
a linear session name; name $\dual{s}$ is the dual of $s$---they are \emph{endpoints} of the same session.
...
$\lambda x.P$ and $\appl{(\lambda x.P)}{a}$, respectively. 
...
we ``pack''
the name to be sent (denoted $b$) into an abstraction; 
upon reception, the receiver ``unpacks'' this object following a precise protocol on a fresh  session (denoted $s$):
%More precisely, our encoding \jpc{of name passing} in \HO is given as:
\begin{align*}
  \map{\bout{a}{b} P}	&= \bbout{a}{ \abs{z}{\,\binp{z}{x} (\appl{x}{b})} } \map{P} \\
...
a challenge in 
 encoding recursion is  
preserving linearity  of session names.
Roughly speaking, 
given $\recp{X}{P}$, 
we encode its recursion body $P$ as an abstraction 
...
through the application of the session names in $\sigma$;  
the other allows us
...
\begin{align*}
   \umap{\binp{a}{x} Q} & = \bbout{a}{\abs{x}{\umap{Q}}} \inact \\
...
is \emph{not type preserving} (cf. \exref{ex:plaus}). Consequently, it is also not \emph{precise}.
...
In \secref{ss:hotel} we recall the \emph{Hotel Booking scenario}, a case study for \HOp that we developed in~\cite{characteristic_bis,KouzapasPY17}.
...
usual constructs for selection and branching, used to specify labeled deterministic choices within sessions~\cite{honda.vasconcelos.kubo:language-primitives}.
Process 
%ny
%$\appl{x}{u}$ 
$\appl{V}{u}$ 
denotes application; it substitutes name $u$ on the abstraction~$V$. 
...
noted $\scong$ and given in \figref{fig:reduction}~(bottom). 
We 
...
% Below we provide essential definitions; see \appref{app:types} 
% %for definitions of type equivalence and type duality, and 
...
\newjb{The type syntax of \HO exclude $\nonhosyntax{C}$ from 
value types $U$; the types of \sessp excludes $\nonpisyntax{L}$ and $\nonpisyntax{\chtype{L}}$.}
Given $\CAL \in \{\HOp, \HO, \pi \}$, 
the sub-calculus 
$\CAL^{-\mathsf{sh}}$ is obtained by 
excluding shared name types ($\chtype{S}$ and $\chtype{L}$), 
from name type $C$.
...
%\jp{
We write 
$\Delta_1\cdot \Delta_2$ for the disjoint union of $\Delta_1$ and $\Delta_2$. 
We write $\Gamma\backslash x$ to denote the environment obtained from $\Gamma$ by removing the assignment $x : \shot{U}$, for some $U$.
Similarly, we write $\Delta_1 \backslash \Delta_2$ and $\Lambda_1 \backslash \Lambda_2$ with the expected reading.
%}
...
\figref{fig:typerulesmys} gives the typing rules.
We now describe some of them; 
%see \appref{app:types} 
see \cite{KouzapasPY17} 
for a full account.
...
\begin{definition}[Balanced Environments]\label{d:wtenv}%\rm
	...
	$s: S_1, \dual{s}: S_2 \in \Delta$ then $S_1 \dualof S_2$ (cf. \defref{def:dual}).
\end{definition}
...
\begin{definition}%[Reduction of Session Environments]%\rm
	\label{def:ses_red}
	%Let  be a session environment.
	We define the relation $\red$ on session environments $\Delta$ as:
	\begin{eqnarray*}
...
types~\cite{KY2015}, whose key notions are summarized next.
...
The typed LTS describes the interaction of well-typed processes with their environment. 
We shall focus on well-typed processes whose type judgements have an empty $\Lambda$, i.e., an empty environment 
for linear higher-order types.
Given this, we write $$\Gamma; \Delta \proves P \by{~\ell~} \Delta' \proves P'$$ to denote 
a (strong) transition with action label $\ell$ (cf. \defref{d:labels} below). % in this LTS. 

Formally, the typed LTS is obtained by coupling an untyped LTS on processes, whose transitions are denoted 
$P \by{\ell} P'$ with a labelled transition relation 
on typing environments, whose transitions are denoted 
$(\Gamma, \Delta) \by{\ell} (\Gamma, \Delta')$ (see \defref{d:tlts}).
These auxiliary LTSs are given in 
\figref{fig:untyped_LTS}
and 
\figref{fig:envLTS}, respectively.
The key idea is that the transitions 
of a typed process should be enabled by its associated typing: % environment:
$$
\text{if } 
P \by{\ell} P' 
\text{ and } 
(\Gamma, \Delta) \by{\ell} (\Gamma, \Delta') 
\text{ then }
...
The LTS on untyped processes, the LTS on typing environments, and the typed LTS share the same set of 
%The interaction of processes with their environment is defined using 
action labels:
%
...
The action for value reception is 
$\bactinp{n}{V}$.
...
\paragraph{A Refined Typed LTS}
The characterisation of barbed congruence relies on a \emph{refined} typed LTS  on typing environments. Intuitively, 
the objective is to have a more stringent rule for input transitions, given as follows:
%
\[
	\tree{
		\dual{s} \notin \dom{\Delta} 
...
The definition of characteristic processes and values is given in \figref{fig:char}.

This refined LTS on typing environments in turn gives rise to a different, refined LTS on processes
(cf. \defref{def:rlts}).
Note the different notation for standard and refined transitions: $ \by{\bactinp{s}{V}}$ and $ \hby{\bactinp{s}{V}}$.
 In the refined LTS, weak transitions are as expected: we write $\Hby{}$ for the reflexive,
transitive closure of $\hby{\tau}$,
$\Hby{~\ell~}$ for %transitions
$\Hby{}\hby{~\ell~}\Hby{}$, and 
$\Hby{\hat{~\ell~}}$ for $\Hby{\ell}$ if
$\ell\not = \tau$ and $\Hby{}$ otherwise.
Further details on the typed LTSs are given in \appref{app:behavioural} and~\cite{KouzapasPY17}.
%}
...
two dual endpoints), while 
 ...
 We write  $\horel{\Gamma}{\Delta}{P}{\hby{\dtau}}{\Delta'}{P'}$ to denote
	...
	See~\secref{ss:deter} and \cite{KouzapasPY17} for formal definitions of $\hby{\btau}$  and $\hby{\stau}$. % rely on an LTS for \HOp;  for details.
 \end{notation}
...
%				Assume $\Gamma; \emptyset; \Delta \proves P \hastype \Proc$. Then 
				$P \red^\ast P'$ then 
				$\Gamma; \Delta \proves P \hwb \Delta'\proves P'$, with $\Delta \red^\ast \Delta'$. 
	\end{enumerate}
\end{lemma}
...
process $\Client_1$, sends two abstractions with body $P_{xy}$, one to each hotel, 
		...
		In $P_{xy}$, % is the mobile code with free names $x, y$:
...
We first define a \emph{typed calculus} parametrised by a process syntax, an operational semantics, and a type system.
Based on this definition, in  
\secref{sec:positive} and \secref{sec:extension}
we will define concrete instances of (higher-order) typed calculi.
...
describing how visible actions in the source calculus $\tyl{L}_1$ are mapped in the target calculus $\tyl{L}_2$.
...
induced by the typing rule for parallel composition (cf. Rule~\textsc{(Par)} in  \figref{fig:typerulesmys}).
...
Recall that (un)typed barbs have been defined in \defref{d:barb}.
...
$\stytraargi{\Gamma}{\tau}{\Delta}{P}{\Delta'}{P'}{1}{1}$
			   then  $\exists Q, \Delta''$ such that \\
 (i)~$\wtytraargi{\mapt{\Gamma}}{}{\mapt{\Delta}}{\map{P}}{\mapt{\Delta''}}{Q}{2}{2}$
			    %(ii)~$\ell_2 = \mapa{\ell_1}$, 
			    and 
				(ii)~${\mapt{\Gamma}};{\mapt{\Delta'}}\proves_2 {\map{P'}}{\,\wb_2\,}
...
%{\mapt{\Delta'}}\proves_2 {Q}$.
			...
			\item	Soundness:   %% Revised
				If  $\wtytraargi{\mapt{\Gamma}}{}{\mapt{\Delta}}{\map{P}}{\mapt{\Delta'}}{Q}{2}{2}$
				then  $\exists P', Q', \Delta'', \Delta'''$ such that  \\
				(i)~$\stytraargi{\Gamma}{\tau}{\Delta}{P}{\Delta''}{P'}{1}{1}$,
				%(ii)~$\ell_2 = \mapa{\ell_1}$, 
				(ii)~$\wtytraargi{\mapt{\Gamma}}{}{\mapt{\Delta'}}{Q}{\mapt{\Delta'''}}{Q'}{2}{2}$
				and \\
				(iii)~
${\mapt{\Gamma}};{\mapt{\Delta''}}\proves_2 {\map{P'}}{\,\wb_2\,}
{\mapt{\Delta'''}}\proves_2 {Q'}$.
...
for our encodings 
 ...
 {(see \propsref{prop:op_corr_HOp_to_HO}, \ref{prop:op_corr_HOp_to_p}, \ref{prop:op_corr_HOpp_to_HOp}, and \ref{prop:op_corr_pHOp_to_HOp}).}
 In particular, we will consider forms of operational correspondence that account also for visible actions, relying on 
 a mapping $\mapa{\cdot}{}$ on actions, as already explained (cf. \defref{def:lopco} below).
...
Also, to compare two precise encodings in \secref{ss:compare} here we introduce the notion of \emph{tight} encodings.
...
The following property, concerning composability of precise encodings, will come in handy in~\secref{sec:extension}.
It follows closely a similar property established in~\cite{DBLP:journals/dc/Gorla10} for (untyped) valid encodings between languages with equivalences which are {reduction-closed}.
...
\begin{proposition}[Composability]%\rm
	...
	Let %encodings 
	$\enco{\pmap{\cdot}{1}, \tmap{\cdot}{1}%, \mapa{\cdot}^{1}
	}: \tyl{L}_1 \to \tyl{L}_2$
	and 
	$\enco{\pmap{\cdot}{2}, \tmap{\cdot}{2}%, \mapa{\cdot}^{2}
	}: \tyl{L}_2 \to \tyl{L}_3$
	be two precise %typed 
	encodings.
	Then their composition, denoted 
	$\enco{\pmap{\cdot}{2} \circ \pmap{\cdot}{1}, \tmap{\cdot}{2} \circ \tmap{\cdot}{1} %, \mapa{\cdot}^{2}\circ \mapa{\cdot}^{1}
	}: \tyl{L}_1 \to \tyl{L}_3$,
	is precise. 
\end{proposition}
...
given in \defref{def:ep}, which is restricted to $\tau$-labelled transitions.
...
\label{sec:positive}
%\input{positive}
In this section,  we  present two %encodability results:
 precise encodings: 
...
We consider the following typed calculi, which result as three instances of~\defref{d:tcalculus}:
%\begin{enumerate}[-]
%	\item
...
The typing $\proves$ is defined in~\secref{sec:types}.
...
and 
$\fwb$ are 
as in \defref{d:hbw} and \defref{d:fwb}.
...
exploits  
 ...
 abstraction
%defined as
$\abs{x_1,\cdots, x_k}{\auxmapp{\pmapp{P}{1}{f}}{{}}{\emptyset} }$, where $x_j = \vmap{n_j}$, for all $j \in \{1, \ldots, k\}$: }
...
The typed encoding 
$\enco{\pmap{\cdot}{1}{f}, \tmap{\cdot}{1} %, \mapa{\cdot}^{1}
}: \tyl{L}_{\HOp} \to \tyl{L}_{\HO}$ is given in 
\figref{f:enc:hopi_to_ho}. 
Mapping $\tmap{\cdot}{1}$ on types homomorphically extends to 
environments $\Delta$
and
$\Gamma$, with
$$
...
%containing $\auxmapp{P}{{}}{\sigma}$ 
is duplicated: 
one copy is used to recover the original recursion body $P$ (through
the application of $\fn{P}$); another copy is used to re-invoke
...
\pmapp{\recp{X}{\bout{a}{m} \varp{X}}}{1}{f} & =  \newspb{s_1}{ 
	...
	\Par
	\binp{s_1}{z_X} 
	\bbout{a}{\abs{z_1}{\binp{z_1}{x} (\appl{x}{m})}} 
	\\
...
& \hby{\tau} 
\newspb{s_2}{
	\binp{s_2}{z_X} \bbout{a}{\abs{z_1}{\binp{z_1}{x} (\appl{x}{m})}} 
	\\
	& \qquad \qquad \qquad 
...
%\begin{example}[Encoding Recursion, OLD]
...
\label{ex:hotel}
The \HOp process $\Client_2$ (cf. \figref{f:hotel}) is one possible implementation for the 
hotel booking scenario described in 
\secref{ss:hotel}. Its encoding in \HO is as follows:
\begin{align*}
...
\end{align*}
where $\pmap{Q_1}{1}_f$ and $\pmap{Q_2}{1}_f$ are given in \figref{ex:hotel}.
\end{example}
...
We now state the properties of the encoding. 
We start with 
...
is type preserving (cf.~\defref{def:tp}) and type sound (cf.~\defref{def:ep}(1)).
\end{proposition}}

\begin{proof}
Type preservation follows directly from \figref{f:enc:hopi_to_ho}.
...
$\enco{\pmapp{\cdot}{1}{f}, \tmap{\cdot}{1} %, \mapa{\cdot}^{1}
...
($\hby{\stau}$ and $\hby{\btau}$, cf. \notref{not:dettrans}) in soundness and completeness statements.
...
the only non trivial issue is encoding higher-order communication, which is present in $\HOp$ but not in 
$\sessp$.}
We closely follow Sangiorgi's encoding~\cite{SangiorgiD:expmpa,SaWabook}, which represents 
...
\secref{ss:hotel} (and \figref{f:hotel}) as the \HOp processes $\Client_1$ and $\Client_2$. 
...
\begin{proof}
Type preservation follows directly from \figref{f:enc:ho_to_sessp}.
...
the mapping on actions $\mapa{\cdot}{2}: \mathcal{A}_{\HOp} \to \mathcal{A}_{\sessp}$ is defined as follows:
	\begin{align*}
		...
		\\
		\mapa{\bactinp{n}{\abs{ x}{P}} }{2} & \defeq \bactinp{n}{m} \quad \quad \text{($m$ fresh)}
	\end{align*}
	and as an homomorphism for other actions $\ell \in \mathcal{A}_{\HOp}$.
\end{definition}
...
\begin{enumerate}[a)]
\item 
the encoding from \HOp to \HO, denoted $\pmapp{\cdot}{1}{f}$ (\secref{subsec:HOpi_to_HO}), translates processes with name passing;
\item 
...
First, by considering the encoding of $P_1$ into \HO  (following mapping $\pmapp{\cdot}{1}{f}$ in \figref{f:enc:hopi_to_ho})
we obtain:
\begin{eqnarray*}
\pmapp{P_1}{1}{f} & = &  	\bbout{s}{\abs{z}{\binp{z}{x} \appl{x}{a}}} \inact \Par \\
& &  \binp{\dual{s}}{y} \newspb{s_0}{\appl{y}{s_0} \Par \bbout{\dual{s_0}}{\abs{x}{(\bout{x}{V_1} \inact \Par \dots \Par \bout{x}{V_n} \inact)}} \inact}\\
	& \hby{\stau} \hby{\btau} & 
	\newsp{s_0}{\binp{s_0}{x} \appl{x}{a} \Par \bbout{\dual{s_0}}{\abs{x}{(\bout{x}{V_1} \inact \Par \dots \Par \bout{x}{V_n} \inact)}} \inact}\\
	& \hby{\stau}\hby{\btau}  & 
	\bout{a}{V_1} \inact \Par \dots \Par \bout{a}{V_n} \inact
\end{eqnarray*}
where we write $V_i$ to stand for $\abs{z}{\binp{z}{x_i} \appl{x_i}{s_i}}$.
Now, we encode $P_2$ into \sessp (following mapping $\pmap{\cdot}{2}$ in~\figref{f:enc:ho_to_sessp}):
\begin{eqnarray*}
...
& & \quad \binp{\dual{s}}{x} \big(\newsp{s_0}{\bout{x}{s_0} \bout{\dual{s_0}}{s_1} \inact} \Par \dots \Par \newsp{s_0}{\bout{x}{s_0} \bout{\dual{s_0}}{s_n}\inact}\big)
	\\
	& \hby{\stau} \equiv & 
	\newspb{a}{\repl{\binp{a}{y} \binp{y}{x} \pmap{R}{2}} \Par 
	\\
	& & \qquad \qquad 
	\newsp{s_0}{\bout{a}{s_0} \bout{\dual{s_0}}{s_1} \inact} \Par \dots \Par \newsp{s_0}{\bout{a}{s_0} \bout{\dual{s_0}}{s_n} \inact}}
	\\
	& \hby{\stau}  \hby{\stau}   & 
	\newspb{a}{\repl{\binp{a}{y} \binp{y}{x} \pmap{R}{2}} \Par \pmap{R}{2}\subst{s_1}{x} \Par 
	\\
	&  & \qquad \qquad \newsp{s_0}{\bout{a}{s_0} \bout{\dual{s_0}}{s_2} \inact} \Par \dots \Par \newsp{s_0}{\bout{a}{s_0} \bout{\dual{s_0}}{s_n} \inact}}
	\\
	& \Hby{}_{2*(n - 1)} & 
	\newspb{a}{\repl{\binp{a}{y} \binp{y}{x} \pmap{R}{2}} \Par \pmap{R}{2}\subst{s_1}{x} \Par \pmap{R}{2}\subst{s_2}{x} \Par \dots \Par \pmap{R}{2}\subst{s_n}{x}}
%	\red
...
we obtain:
\begin{eqnarray*}
	\pmapp{Q_1}{1}{f} & = & \bout{s'}{\abs{z}{\binp{z}{x} \appl{x}{s}}} \inact \Par 
...
encoding  $\pmapp{\cdot}{1}{f}$ in the presence of linearity should, however, be considered in a broader setting, for 
in \secref{ss:negative} we prove that linear resources do 
not suffice to encode shared communications.
Therefore, in the general case featuring linear and shared communication not only the benefits of  $\pmap{\cdot}{2}$ over  $\pmapp{\cdot}{1}{f}$ could not be obtained,
but the drawbacks mentioned in the comparison between $\pmapp{P_1}{1}{f}$ and $\pmap{P_2}{2}$ (i.e., the garbage processes 
generated by $\pmap{\cdot}{2}$) 
could well be more prominent. This observation may be used to informally argue that $\pmapp{\cdot}{1}{f}$ is ``better than'' $\pmap{\cdot}{2}$ (or, alternatively, that \HOp is closer to \HO than to \sessp);
...
%we introduce an 
%additional notion of encoding (\emph{tight encodings}) which satisfies an 
%extra correctness criterion: a form of operational correspondence 
%for \emph{visible actions}. 
%
%As already motivated above, 
...
%To formally distinguish between $\map{\cdot}_f^1$ and $\pmap{\cdot}{2}$
To  formally state that 
\HOp and \HO are more closely related than \HOp and~\sessp,
we may 
distinguish the precise encodings $\pmapp{\cdot}{1}{f}$ and $\pmap{\cdot}{2}$ depending on whether they are also tight encodings or not (cf. \defref{def:lopco}):
...
The proof proceeds by showing that the encoding $\pmapp{\cdot}{1}{f}$ enjoys 
labelled operational correspondence, whereas $\pmap{\cdot}{2}$ does not. 
Recall that a labeled operational correspondence for  $\pmapp{\cdot}{1}{f}$  has been
already 
stated in 
\propref{prop:op_corr_HOp_to_HO}.
The analog of \propref{prop:op_corr_HOp_to_HO} does not hold for the encoding  $\pmap{\cdot}{2}$  of \HOp into \sessp.
...
\begin{proof}
	...
	have the same barbs.
	Now, by $\tau$-inertness (\lemref{lem:tau_inert}), we have both 
	\begin{eqnarray}
...
typed encodings of \HOp into \HO (cf.~\defref{d:enc:hopitoho}) and of 
\HO into \sessp
	(cf.~\defref{d:enc:hopitopi}), which work uniformly for linear and shared names, 
	as well as 
	from the preciseness results for such encodings
	(cf. \propref{f:enc:hopitoho} and~\propref{f:enc:hotopi}).
\end{proof}
}
...
abstractions/applications and polyadicity (cf. \cororef{coro:ho} and \cororef{coro:poly})

We present precise encodings of \HOpp and \PHOp into \HOp.
We then use the encodings of \HOp into \HO and \sessp in the previous section, 
together with 
...
These types can be easily accommodated in the type system 
in \secref{sec:types}:
in \figref{fig:typerulesmys}, 
we replace $C$ by $U$ in Rule~\textsc{(Abs)} and $C$ by $U'$ in Rule~\textsc{(App)}. 
...
%\figref{fig:typerulesmy} 
\secref{sec:types}
with Rules \textsc{(Abs)} and \textsc{(App)} modified as explained above.
Formally, the set   
...
The typed encoding
$\enco{\pmap{\cdot}{3}, \tmap{\cdot}{3}}: \tyl{L}_\HOpp \to \tyl{L}_{\HOp}$ is defined
in \figref{f:enc:hopip_to_hopi}.
\end{definition}

We consider mappings for terms and types, denoted $\pmap{\cdot}{3}$ and $\tmap{\cdot}{3}$, respectively.
...
\begin{align*}
& \pmap{\bout{s}{\abs{x}{Q}}\inact  \Par \binp{\dual{s}}{y}(\appl{\abs{z}{(\appl{z}{s_1}})}{y})}{3} 
\\
=~ &
\bout{s}{\underline{\auxmap{\abs{x}{Q}}{3}}}\underline{\pmap{\inact}{3}}  \Par \binp{\dual{s}}{y}\underline{\pmap{(\appl{\abs{z}{(\appl{z}{s_1}})}{y})}{3}} 
\\
...
\begin{proof}
Type preservation follows directly from \figref{f:enc:hopip_to_hopi}.
...
Given 
the typed encoding
$\enco{\pmap{\cdot}{3}, \tmap{\cdot}{3}}: \tyl{L}_\HOpp \to \tyl{L}_{\HOp}$ (cf. \defref{d:enc:hopptohop}),
...
\begin{enumerate}[-]
\item $\enco{\pmap{\cdot}{1}_f, \tmap{\cdot}{1} %, \mapa{\cdot}^{1}
}: \tyl{L}_{\HOp} \to \tyl{L}_{\HO}$  (cf. \defref{d:enc:hopitoho})
\item $\enco{\pmap{\cdot}{2}, \tmap{\cdot}{2} %, \mapa{\cdot}^{2}
}: \tyl{L}_{\HOp} \to \tyl{L}_{\sessp}$ (cf. \defref{d:enc:hopitopi})
\item $\enco{\pmap{\cdot}{3}, \tmap{\cdot}{3}}: \tyl{L}_{\HOpp} \to \tyl{L}_{\HOp}$ (cf. \defref{d:enc:hopptohop})
\end{enumerate}
...
The operational semantics in \figref{fig:reduction} requires only minor modifications to accommodate the simultaneous substitution $\subst{\widetilde{V}}{\widetilde{x}}$ (for equally sized $\widetilde{u}/\widetilde{V}$ and $\widetilde{x}$)
...
$\mathcal{A}_{\PHOp}$ extends that in \defref{d:labels} 
with actions
...
\begin{proof}
Type preservation follows directly from \figref{f:enc:poltomon}.
...
Consider the typed encodings
\begin{enumerate}[-]
\item $\enco{\pmapp{\cdot}{1}{f}, \tmap{\cdot}{1} %, \mapa{\cdot}^{1}
}: \tyl{L}_{\HOp} \to \tyl{L}_{\HO}$  (cf. \defref{d:enc:hopitoho})
\item $\enco{\pmap{\cdot}{2}, \tmap{\cdot}{2} %, \mapa{\cdot}^{2}
}: \tyl{L}_{\HOp} \to \tyl{L}_{\sessp}$ (cf. \defref{d:enc:hopitopi})
\item $\enco{\pmap{\cdot}{4}, \tmap{\cdot}{4}}: \tyl{L}_{\PHOp} \to \tyl{L}_{\HOp}$ (cf. \defref{d:enc:phoptohop})
\end{enumerate}
Then the following typed encodings 	are precise:
\begin{enumerate}[-]
\item 
...
and
casts known results~\cite{SangiorgiD:expmpa} into a session typed setting.
...
We have shown in \secref{subsec:HOp_to_sessp} that the analogue of Sangiorgi's encoding for the session-typed setting also enjoys full abstraction (up to $\hwb$/$\fwb$, cf. \propref{prop:op_corr_HOp_to_p}).
A basic form of input/output types is used in~\cite{DBLP:journals/tcs/Sangiorgi01}, where the encoding in~\cite{SangiorgiD:expmpa} is cast in the asynchronous setting, with output and applications coalesced in a single construct. Building upon~\cite{DBLP:journals/tcs/Sangiorgi01}, 
a simply typed encoding for synchronous processes is given in~\cite{SaWabook}; the reverse encoding (i.e.,  first-order communication into higher-order processes) is also studied  for an asynchronous, localised $\pi$-calculus (where only the output capability of names can be sent around).
The work~\cite{San96int} studies hierarchies for calculi with \emph{internal} first-order mobility and 
with higher-order mobility without name-passing (similarly as in \HO); these hierarchies are
defined according to the order of types needed in typing.
Via fully abstract encodings, it is shown that name- and process-passing calculi with equal order of types have the same expressiveness.
...
\newj{The work \cite{OY2016} studies two encodings:
...
We have thoroughly studied the expressivity of the higher-order $\pi$-calculus with sessions,
here denoted $\HOp$.
To this end, we developed a new abstract notion of (precise) encoding that accounts for (session) types in both source and target calculi.
Indeed, unlike most previous works, 
%on the expressivity of (higher-order) process calculi, 
we have carried out our expressiveness study in the setting of \emph{session types}. % for structured communications.
Types not only delineate and enable encodings; they 
inform the techniques required to reason about their correctness properties.
...
We are grateful to %ESOP'16 \
Alen Arslanagi\'{c}
and to the
anonymous reviewers for their useful remarks and suggestions.

This work has been partially sponsored by the Doctoral Prize Fellowship, EPSRC EP/K011715/1, EPSRC EP/K034413/1, and EPSRC EP/L00058X/1, EU project FP7-612985 UpScale, and EU COST Actions IC1201 (BETTY), IC1402 (ARVI), IC1405 (Reversible Computation), and CA15123 (EUTypes). 
...
P\'{e}rez has been partially supported by the Netherlands Organization for Scientific Research (NWO) under the VIDI Project No. 016.Vidi.189.046 (Unifying Correctness for Communicating Software). 
He is also affiliated to the NOVA Laboratory for Computer Science and Informatics (NOVA LINCS), Universidade Nova de Lisboa, Portugal.
