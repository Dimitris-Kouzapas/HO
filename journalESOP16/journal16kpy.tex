...
%Notice that for open process terms (i.e., with free variables),  we can always apply Rule~$\trule{EProm}$ (cf. \figref{fig:typerulesmy}) and obtain an empty $\Lambda$. 
Indeed, we will be working with closed processes, i.e., processes without free  variables.
Since $\Lambda$ is empty, 
\newjb{ % (a visible action or $\tau$),
we write $\Gamma; \Delta \proves P \hby{\ell} \Delta' \proves P'$ to denote 
a (strong) transition with label $\ell$. % in this LTS. 
...
further details on the typed LTS are given in \appref{app:behavioural} and~\cite{KouzapasPY17}.
