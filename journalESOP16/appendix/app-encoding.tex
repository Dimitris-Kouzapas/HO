...
\begin{proposition}[Full Abstraction, \HOp into \HO]\rm
	...
	if and only if
	$\horel{\tmap{\Gamma}{1}}{\tmap{\Delta_1}{1}}{\pmapp{P_1}{1}{f}}{\hwb}{\tmap{\Delta_2}{1}}{\pmapp{Q_2}{1}{f}}$.
\end{proposition}

\dk{For the proof of the Full Abstraction theorems we are based on the detailed labelled correspondence to create
bisimulation closures up-to determinacy (\ref{lem:up_to_deterministic_transition}). Proving the closure is almost
trivial for most of the cases, where we just follow the requirements of the labelled correspondence transitions.
The interesting cases are the output cases, where the cases need to be analysed up-to the characteristic process.}
...
%
	\[
		\Re = \set{\horel{\Gamma}{\Delta_1}{P_1}{\hwb}{\Delta_2}{Q_1} \setbar \horel{\mapt{\Gamma}^{1}}{\mapt{\Delta_1}^{1}}{\pmapp{P_1}{1}{f}}{\hwb}{\mapt{\Delta_2}^{1}}{\pmapp{Q_1}{1}{f}}}
	\]
%
	\noi	The proof considers a case analysis on the transition $\hby{\ell}$ and
		...
		We give the interesting output case. The others are similar of easier.
...
%
	...
	\]
%
	\noi Based on the encoding \pmapp{\cdot}{1}{f}, we rewrite the last result as
	\[
...
%	(\propref{prop:op_corr_HOp_to_p}).
%
	Proof follows directly from \propref{app:prop:op_corr_HOp_to_p}.
	The cases of \propref{app:prop:op_corr_HOp_to_p} are used to create a
	...
	\noi {\bf Part 1: Soundness}

	\noi Consider relation
	\[
		\mathcal{R} = \set{\horel{\Gamma}{\Delta_1}{P}{,}{\Delta_2}{Q} \setbar \horel{\mapt{\Gamma}^{2}}{\mapt{\Delta_1}^2}{\pmap{P}{2}}{\fwb}{\mapt{\Delta_2}^2}{\pmap{Q}{2}}}
	\]
%
	The interesting case is the output case. The rest of the cases follow
	the bisimulation game that is implied by the labelled correspondence Proposition~\ref{app:prop:op_corr_HOp_to_p}.

	\noi $P = \bout{n}{\abs{x}{R_1}} P'$

	\noi We then observe transition
%
	\begin{eqnarray*}
		\Gamma_1; \Delta_1 \proves P \hby{\bactout{n}{\abs{x}{R_1}}} \Delta_1' \proves P'
	\end{eqnarray*}
%
	From Proposition~\ref{app:prop:op_corr_HOp_to_p} (Part 1), we have that
%
	\begin{eqnarray*}
	...
	\end{eqnarray*}
%
	which implies from the requirements of $\fwb$ that 
%
	\begin{eqnarray}
...
%
	...
	\noi From~\ref{prop:fa_ho_sessp_1} and Proposition~\ref{app:prop:op_corr_HOp_to_p} (Part 1), we get that
...
%
	...
	\end{eqnarray*}
%
	If we follow the bisimulation transitions, that are determinate transitions, on the last bisimulation pair we get that
	...
	{\bf Part 2. Completeness}

	\noi Consider relation
	\[
		\mathcal{R} = \set{ \horel{\mapt{\Gamma}^{2}}{\mapt{\Delta_1}^2}{\pmap{P}{2}}{,}{\mapt{\Delta_2}^2}{\pmap{Q}{2}} \setbar \horel{\Gamma}{\Delta_1}{P}{\fwb}{\Delta_2}{Q}}
	\]
%
	The interesting case is the output case. The rest of the cases follow
	the bisimulation game that is implied by the labelled correspondence
	Proposition~\ref{app:prop:op_corr_HOp_to_p}.
...
%
	...
	\end{eqnarray*}
%
	to get from Proposition~\ref{app:prop:op_corr_HOp_to_p} (Part 2),
	...
	\noi
	Also from Proposition~\ref{app:prop:op_corr_HOp_to_p} (Part 2) we get that,
	...
	\noi
	and also from the definition of $\mathcal{R}$ and the fact that the pair
	of mapped processes can observe only determinate transitions we get
...
%
	...
	\begin{enumerate}
		...
		\item	If $\ell \in \set{\news{\tilde{m}} \bactout{n}{\abs{x}{R}}, \bactinp{n}{\abs{x}{R}}}$
				...
				and
				\[
				\horel{\tmap{\Gamma}{4}}{\tmap{\Delta_1'}{4}}{P_1 \Par C}{\wb}{\tmap{\Delta_2'}{4}}{Q_1 \Par C}
				\]
...
%	\dk{Is the proof easy? do the proof}
	...
	\begin{enumerate}
		...
		and
		\begin{eqnarray*}
			\horel{\tmap{\Gamma}{4}}{\tmap{\Delta_1'}{4}}{\pmap{P' \Par C_1 \Par C_2}{4}}{\wb}{\tmap{\Delta_2'}{4}}{\pmap{Q' \Par C_1 \Par C_2}{4}}
		\end{eqnarray*}
