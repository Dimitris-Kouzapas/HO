...
recall that we prove a stronger statement than that in \defref{def:ep}(2),
		as we follow 
		\defref{def:lopco}.
For full abstraction, we rely on the following notational convention:

\begin{notation}[Typed Relations]
For the sake of readability, when describing typed relations we shall omit typing information, which is usually clear from the context. This way, e.g., in the proof of
...
Recall that we use the mapping on actions $\mapa{\cdot}^{1}$ given in \defref{d:actmap1}.
