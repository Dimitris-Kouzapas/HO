...
Suppose %$\horel{\Gamma}{\Delta_1}{P_1}{\hby{\ell}}{\Delta_1'}{P_2}$; 
$\stytra{\mapt{\Gamma}^{2}}{\ell_2}{\mapt{\Delta_1}^{2}}{\map{P}^{2}}{\mapt{\Delta_1'}^{2}}{R}$;
we perform a case analysis on the shape of ${\ell}$, using 
\propref{app:prop:op_corr_HOp_to_p} (Part 2).
		The  most interesting case is when $\ell = \bactout{n}{\abs{x}{R_1}}$, with  {$\fs{R_1} = \emptyset$}; the other cases are similar or simpler. 
